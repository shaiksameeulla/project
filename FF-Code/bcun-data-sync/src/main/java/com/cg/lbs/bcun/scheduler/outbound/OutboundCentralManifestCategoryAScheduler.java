 ...
 */
public class OutboundCentralManifestCategoryAScheduler extends QuartzJobBean  {

	/**
	 * Used to log the message
	 */
	private static final Logger logger = LoggerFactory.getLogger(OutboundCentralManifestCategoryAScheduler.class);
	...
	protected void executeInternal(JobExecutionContext arg0)
		...
		logger.info("OutboundCentralManifestCategoryAScheduler::executeInternal::start=====>");
		//Starting process
		try {
			dataProcessor.proceedOutbountDataPreparation(BcunConstant.OUTBOUND_OFFICE_CATEGORY_A);
		}catch (HttpException e) {
			logger.error("OutboundCentralManifestCategoryAScheduler::executeInternal::HttpException::" , e);
		} catch (ClassNotFoundException e) {
			logger.error("OutboundCentralManifestCategoryAScheduler::executeInternal::ClassNotFoundException::" , e);
		} catch (IOException e) {
			logger.error("OutboundCentralManifestCategoryAScheduler::executeInternal::IOException::" , e);
		} catch(Exception e) {
			logger.error("OutboundCentralManifestCategoryAScheduler::executeInternal::Exception::" , e);
		}
		logger.info("OutboundCentralManifestCategoryAScheduler::executeInternal::end=====>");
	}
