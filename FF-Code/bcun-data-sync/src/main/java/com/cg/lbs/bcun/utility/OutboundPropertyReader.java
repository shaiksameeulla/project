 ...
 * 
 */
public class OutboundPropertyReader {

	
	private static final Logger logger = Logger.getLogger(OutboundPropertyReader.class);
	...
	static {
		try {
			configPropertyFile = ResourceBundle.getBundle("OutboundConfig");
			configToList = readConfigInfoFromProperties();
			Collections.sort(configToList);
		} catch (Exception e) {
			logger.error("OutboundPropertyReader::static block::error in loading property file:e:getMessage:" , e);
		}
	...
	public  static List<OutboundConfigPropertyTO> getInboundConfigProperty() {
	...
	public  static OutboundConfigPropertyTO getInboundConfigPropertyByProcessName(String processName) {
		...
		try {
			...
			if(propKeys != null && !propKeys.isEmpty()) {
				...
				for (String key : propKeys) {
					logger.trace("OutboundPropertyReader::readConfigInfoFromPropertiese:key["+key+"]" );
					OutboundConfigPropertyTO to = getInboundConfigPropertyByProcessName(key);
					to.setPropKey(key);
					configToList.add(to);
				}
			}
		} catch (Exception e) {
			logger.error("OutboundPropertyReader::readConfigInfoFromProperties::error in reading property file");
			logger.error("OutboundPropertyReader::readConfigInfoFromProperties::error:e:getMessage:" , e);
		}
		return configToList;
	}
		...
		if(propValue != null && !propValue.isEmpty()) {
			...
			if (tokens != null && tokens.length > 0) {
				for (String token : tokens) {
					logger.trace("OutboundPropertyReader::prepareConfigTo::token:: " + token);
					if(token != null && !token.isEmpty() ) {
						try {
							...
							logger.error("OutboundPropertyReader::prepareConfigTo::error:: for token ["+ token + "] Exception:e:getMessage:" , e);
							//logger.error("OutboundPropertyReader::prepareConfigTo::error:: for token ["+ token + "] Exception:e:getCause:getLocalizedMessage:" + e.getCause().getLocalizedMessage());
						}
					}
				}
			}
	...
	private static OutboundConfigPropertyTO setConfigTO(String[] keyValue,
		...
		logger.trace("OutboundPropertyReader::setConfigTO::token name [" + key + "] and token value [" + value + "]");
	...
	public static List<OutboundConfigPropertyTO> getOutBoundPropertyListByProcess(String processkey){
		if(CGCollectionUtils.isEmpty(configToList)){
			configToList=readConfigInfoFromProperties();
		}
