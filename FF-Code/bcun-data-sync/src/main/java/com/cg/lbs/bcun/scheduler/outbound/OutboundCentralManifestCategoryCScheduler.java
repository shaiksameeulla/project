 ...
 */
public class OutboundCentralManifestCategoryCScheduler extends QuartzJobBean  {

	/**
	 * Used to log the message
	 */
	private static final Logger logger = LoggerFactory.getLogger(OutboundCentralManifestCategoryCScheduler.class);
	...
	protected void executeInternal(JobExecutionContext arg0)
		...
		logger.info("OutboundCentralManifestCategoryCScheduler::executeInternal::start=====>");
		//Starting process
		try {
			dataProcessor.proceedOutbountDataPreparation(BcunConstant.OUTBOUND_OFFICE_CATEGORY_C);
		} catch (HttpException e) {
			logger.error("OutboundCentralManifestCategoryCScheduler::executeInternal::HttpException::" , e);
		} catch (ClassNotFoundException e) {
			logger.error("OutboundCentralManifestCategoryCScheduler::executeInternal::ClassNotFoundException::" , e);
		} catch (IOException e) {
			logger.error("OutboundCentralManifestCategoryCScheduler::executeInternal::IOException::" , e);
		} catch(Exception e) {
			logger.error("OutboundCentralManifestCategoryCScheduler::executeInternal::Exception::" , e);
		}
		logger.info("OutboundCentralManifestCategoryCScheduler::executeInternal::end=====>");
	}
