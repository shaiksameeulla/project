		...
		if(error!=null){
			...
			if(isNull(assignApproverTO.mappingTo)){
				...
				if(!isNull(officeTOList)){
					...
					if(!isNull(assignApproverTO.officeAry)){
			...
			if(!isNull(assignApproverTO.applScreenAry)){
				...
				var scrnLen = assignApproverTO.applScreenAry.length;
				for ( var k = 0; k < scrnLen; k++) {
				for ( var j = 1; j <= boxLen; j++) {
...
function populateOfficeDetails(data) {
	...
	if (!isNull(data)) {
		...
		if(error!=null){
			...
			officeList = responseText[SUCCESS_FLAG];
			if(!isNull(officeList)){
				getDomElementById(office).disabled = false;
