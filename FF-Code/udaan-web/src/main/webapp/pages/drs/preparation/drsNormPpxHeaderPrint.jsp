...
<script type="text/javascript" charset="utf-8"
	src="js/drs/preparation/drsNormPriorityDoxPrint.js"></script>
</head>
...
<table cellpadding="2" cellspacing="0" width="100%" border="0">
		<tr>
			<td align="left" valign="top">
				...
				</center> <%-- <center><b>   ${drsTo.createdOfficeTO.officeName}</b><br/></center> --%>
				<center>
					<bean:message key="label.print.drs.normPpxHeader" />
				</center>
			</td>
		</tr>
		<tr>
				...
				<table cellpadding="2" cellspacing="5" width="100%" border="0">
					...
					<tr>
						<td width="10%" align="left"><bean:message
								key="label.print.drs.type" /></td>
						<td width="8%" align="left">${drsTo.drsFor} <c:if
								test="${not empty drsTo.franchiseTO } ">
								<c:out value="${drsTo.franchiseTO.customerCode}" />
							</c:if> <c:if test="${not empty drsTo.fieldStaffTO } ">
								<c:out value="${drsTo.fieldStaffTO.empCode}" />
							</c:if> <c:if test="${not empty drsTo.coCourierTO } ">
								<c:out value="${drsTo.coCourierTO.vendorCode}" />
							</c:if> <c:if test="${not empty drsTo.baTO } ">
								<c:out value="${drsTo.baTO.customerCode}" />
							</c:if></td>
						<%-- <th width="10%" align="left" ><bean:message key="label.print.dateTime" /></th>
                	<td width="18%" align="left" >${drsTo.drsDate}</td>  --%>
						<td width="10%" align="left"><bean:message
								key="label.pickup.customer.name" /></td>

						<c:if test="${not empty drsTo.fieldStaffTO}">
							<td width="8%" align="left" valign="top"><c:out
                	...
                	<td width="16%" align="left" >${drsTo.fieldStaffTO.firstName} ${drsTo.fieldStaffTO.lastName}</td> --%>
						<td width="10%" align="left"><bean:message
								key="label.print.drs.timeOut" /></td>
						<td width="8%" align="left">${drsTo.fsOutAlias}</td>
						<td width="2%" align="left"><bean:message
