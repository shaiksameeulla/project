...
function addAdditionalPriorityColumns() {
	b[0] = '<select  id="pr_stateId'
			+ rowCountforpr
			+ '" name="to.baPrioritySlabRateTO.stateId" class="selectBox width155" onchange="getCityByState(this,'+rowCountforpr+',\'pr_cityIds\');"/>';
	b[1] = '<select id="pr_cityIds'
			+ rowCountforpr
			+ '" name="to.baPrioritySlabRateTO.cityIds" class="selectBox width155" onchange="validateCities(\'PR\','+ rowCountforpr+',\'pr_cityIds\',\'pr_stateId\');"/>';
	...
	for ( var i = 2; i < prioritySplDestcoloumCount; i++) {
		if (i + 1 == prioritySplDestcoloumCount) {
			b[i] = '<input type="text" name = "to.baPrioritySlabRateTO.specialDestinationRate" id="pr_specialDestinationRate'
					+ rowCountforpr
					+ i
					+ '" class="txtbox width90" onfocus="validateStates(\'PR\','+rowCountforpr+',\'pr_cityIds\',\'pr_stateId\');" onblur="addPrioritySpecialDestRow('+rowCountforpr+',\'PR\');"/>'
			...
			b[i] = '<input type="text" name = "to.baPrioritySlabRateTO.specialDestinationRate" id="pr_specialDestinationRate'
					+ rowCountforpr
					+ i
					+ '" class="txtbox width90" onfocus="validateStates(\'PR\','+rowCountforpr+',\'pr_cityIds\',\'pr_stateId\');"/>'
					+ '<input type="hidden" id="pr_specialDestinationId'
					+ rowCountforpr
					+ i
					+ '" name="to.baPrioritySlabRateTO.specialDestinationId" />';
...
function saveOrUpdateSlabsForPriority() {
	...
	if ((!isNull(headId) || (isNull(headId) && validateDates("save"))) 
			&& isValidInput() 
			&& checkRateGridValues(baPrioritySectorList, baPriorityWtSlabList, priorityProductId, "PR")
			&& checkSpecialDestRateGridValues("PR")) {
		enableHeaderBeforeSave();
		
		disEnableSector(baPrioritySectorList, baPriorityWtSlabList, priorityProductId, "PR", false, false);
...
function callsearchBARateConfigurationForPriority(ajaxResp, servicedOn) {
	...
	disEnableSector(baPrioritySectorList, baPriorityWtSlabList, productId, "PR", true, false);
			...
			if (sectorList[i].sectorType == "D"
				...
				for ( var j = 0; j < weightSlabList.length; j++) {
					if (weightSlabList[j].rateCustomerProductCatMapTO.rateProductCategoryTO.rateProductCategoryId == productId) {
						...
						if(!isNull(getDomElementById("rateid"
								+ productId
								+ sectorList[i].sectorTO.sectorId
								+ weightSlabList[j].weightSlabTO.weightSlabId))){
						getDomElementById("rateid"
								+ productId
								+ sectorList[i].sectorTO.sectorId
								+ weightSlabList[j].weightSlabTO.weightSlabId).value = "";
						}
		...
		for ( var p = 0; p < productTOList.length; p++) {
			...
			// deleteSpecialDestPriority();
				...
				/*for ( var k = 0; k < weightSlabList.length; k++) {
					l =0;
					for ( var j = 0; j < destspecialSlabsList.length; j++) {
						if ((destspecialSlabsList[j].servicedOn == servicedOn) && destspecialSlabsList[j].weightSlabId == weightSlabList[k].weightSlabTO.weightSlabId								
								&& weightSlabList[k].rateCustomerProductCatMapTO.rateProductCategoryTO.rateProductCategoryId == productId) {
							if(!isNull(getDomElementById("pr_stateId" + (l + 1)))){
								getDomElementById("pr_stateId" + (l + 1)).value = destspecialSlabsList[j].stateId;
									//getDomElementById("pr_city" + (l + 1)).value = destspecialSlabsList[j].cityTO.cityName;
									loadCityDropDown("pr_cityIds" + (l+ 1), "", destspecialSlabsList[j].cityList);
									getDomElementById("pr_cityIds" + (l+ 1)).value = destspecialSlabsList[j].cityTO.cityId;
										l++;
							}
						}
						if(l==(rowCnt)){
				...
				//var splSlabs = specialSlabsList[i].specialSlabs;
						...
						for ( var j = 0; j < destspecialSlabsList.length; j++) {
							...
							if ((destspecialSlabsList[j].servicedOn == servicedOn) && destspecialSlabsList[j].weightSlabId == weightSlabList[k].weightSlabTO.weightSlabId								
								...
								if(getDomElementById("pr_stateId" + (r + 1)).value == destspecialSlabsList[j].stateId
										&& (isNull(destspecialSlabsList[j].cityTO) || ((!isNull(destspecialSlabsList[j].cityTO)) && (getDomElementById("pr_stateId" + (r + 1)).value == destspecialSlabsList[j].cityTO.cityId)))){
								getDomElementById("pr_specialDestinationRate"
										+ (r + 1) + (k + 2)).value = destspecialSlabsList[j].rate;
								getDomElementById("pr_specialDestinationId"
										+ (r + 1) + (k + 2)).value = destspecialSlabsList[j].specialDestinationId;
								m++;
									}
								}
								
								}
							}if(m == weightSlabList.length){
								l++;
							}
						}*/
				...
				for(var r=1; r<=rowCnt; r++){			
					...
					for(var k=0;k<destspecialSlabsList.length;k++){
						isExist = false;
						if(((isNull(servicedOn) && isNull(destspecialSlabsList[k].servicedOn)) || (!isNull(servicedOn) && !isNull(destspecialSlabsList[k].servicedOn) &&  (destspecialSlabsList[k].servicedOn == servicedOn)))){
							if(c==1){
								getDomElementById("pr_stateId"+r).value = destspecialSlabsList[k].stateId;
								
								clearCityDropDownList("pr_cityIds"+r);	
								
								loadCityDropDown("pr_cityIds"+r, "", destspecialSlabsList[k].cityList);
								...
								for(var b=1;b<c;b++){
									if(getDomElementById("pr_stateId"+b).value == destspecialSlabsList[k].stateId 
											&& (isNull(destspecialSlabsList[k].cityTO) || ((!isNull(destspecialSlabsList[k].cityTO)) && (getDomElementById("pr_cityIds"+b).value == destspecialSlabsList[k].cityTO.cityId)))){
										isExist = true;
										break;
									}
								}
					...
					for(var j = 0; j<weightSlabList.length ; j++){
						...
						city = getDomElementById("pr_cityIds"+r).value;
							...
							for(var k=0;k<destspecialSlabsList.length;k++){
								if((state == destspecialSlabsList[k].stateId) && ((isNull(city) && isNull(destspecialSlabsList[k].cityTO)) || ((!isNull(city) && city == destspecialSlabsList[k].cityTO.cityId))) 
										&& (destspecialSlabsList[k].weightSlabId == weightSlabList[j].weightSlabTO.weightSlabId)
										&& ((isNull(servicedOn) && isNull(destspecialSlabsList[k].servicedOn)) || (!isNull(servicedOn) && !isNull(destspecialSlabsList[k].servicedOn) &&  (destspecialSlabsList[k].servicedOn == servicedOn)))){
								getDomElementById("pr_specialDestinationRate"+n+m).value = destspecialSlabsList[k].rate;
	...
	if (!isNull(baRateHeader)) {
		...
		if (!isNull(rtoCharges)) {
			...
			if (rtoCharges.rtoChargeApplicable == "Y") {
				...
				//document.getElementById("isApplicableForPriority").value = rtoCharges.rtoChargeApplicable;
			...
			for ( var k = 0; k < weightSlabList.length; k++) {
				for ( var j = 0; j < splSlabs.length; j++) {
					if (splSlabs[j].weightSlabId == weightSlabList[k].weightSlabTO.weightSlabId) {
						//getDomElementById("pr_city" + (i + 1)).value = splSlabs[j].cityTO.cityName;
