...
function addAdditionColumns() {
	a[0] = '<select  id="stateId'
			+ rowCount
			+ '" name="to.baCourierSlabRateTO.stateId" class="selectBox width155" onchange="getCityByState(this,'+rowCount+',\'cityIds\');"/>';			
	a[1] = '<select id="cityIds'+rowCount+'" name="to.baCourierSlabRateTO.cityIds" class="selectBox width155" onchange="validateCities(\'CO\','+rowCount+',\'cityIds\',\'stateId\');"/>';
	...
	for ( var i = 2; i < courierSplDestcoloumCount; i++) {
		if (i + 1 == courierSplDestcoloumCount) {
			a[i] = '<input type="text" name = "to.baCourierSlabRateTO.specialDestinationRate" id="specialDestinationRate'
					+ rowCount
					+ i
					+ '" class="txtbox width90" onfocus="validateStates(\'CO\','+rowCount+',\'cityIds\',\'stateId\');" onblur="addCourierSpecialDestRow('+rowCount+',\'CR\');"/>'
			...
			a[i] = '<input type="text" name = "to.baCourierSlabRateTO.specialDestinationRate" id="specialDestinationRate'
					+ rowCount
					+ i
					+ '" class="txtbox width90" onfocus="validateStates(\'CO\','+rowCount+',\'cityIds\',\'stateId\');"/>'
					+ '<input type="hidden" id="specialDestinationId'
					+ rowCount
					+ i
					+ '" name="to.baCourierSlabRateTO.specialDestinationId" />';
	...
	if (!isNull(baRateHeader)) {
		...
		if (!isNull(rtoCharges)) {
			...
			if (rtoCharges.rtoChargeApplicable == "Y") {
				...
				//document.getElementById("isApplicableForCourier").value = rtoCharges.rtoChargeApplicable;
