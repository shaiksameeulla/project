...
function setSplDestRate(prodCatId, prodCode, rows, cols, splDest, servicedOn) {
	...
	for(var r=1; r<=rows; r++){
		...
		if(prodCode == "CO"){
			...
			if(rows != (rowCount_CO-2) && rows!=1){
				addCourierSplDestRow();
			}	
		}else if(prodCode == "TR"){
			
			if(rows != (rowCount_TR-2)&& rows!=1){
				addTrainSplDestRow();
			}	
		}else if(prodCode == "AR"){
			
			if(rows != (rowCount_AR-2)&& rows!=1){
				addAirCargoSplDestRow();
			}	
		}else if(prodCode == "PR"){
			if(rows != (rowCount_PR-2)&& rows!=1){
				addPrioritySplDestRow();
			}	
		}
		...
		for(var j = 1; j<=cols ; j++){
			...
			if(!isNull(wtSlab)){
				...
				for(var k=0;k<splDest.length;k++){
					if((state == splDest[k].stateId) && ((isNull(city) && isNull(splDest[k].cityTO)) || ((!isNull(city) && city == splDest[k].cityTO.cityId))) 
