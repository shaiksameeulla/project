...
var rowCount = 1;
/** refer drsCommon.js */
tableId='drsUpdateNpPpx';
/** refer drsCommon.js */
formId='npDrsForm';
/** refer drsCommon.js */
savePromptMsg="Do you want to Update DRS ?";
/** for Clear screen */
pageLoadUrl= "./updateNormPpxDrs.do?submitName=viewUpdateDrsPage";
/** called on Load */
...
function find(){
	if(validationsForFind()){
		var url="./updateNormPpxDrs.do?submitName=findDrsNumberForUpdate";
...
function disableGrid(){
	...
	//	var deliveryTimeDom=  document.getElementsByName("to.rowDeliveryTime");
		...
		var compnySealSig= document.getElementsByName("to.rowCompanySealSign");
		...
		for(var i=0;i<deliveryTypeDom.length;i++) {
			...
			var consgNumber=consgDom[i].value;
			if(getCnoteProduct(consgNumber)==getQseries()){
				
				relationDom[i].disabled=false;
				contactNumDom[i].setAttribute("onkeypress","enterKeyNav('rowCompanySealSign"+(i+1)+"'\,event.keyCode)");
				compnySealSig[i].setAttribute("onkeypress","enterKeyNav('rowRelationId"+(i+1)+"'\,event.keyCode)");
				relationDom[i].setAttribute("onkeypress","enterKeyNavforDrsUpdateContactNum(event.keyCode,"+"'"+(i+1)+"'"+",'rowRelationId')");
				
			}else{
				relationDom[i].disabled=true;
				relationDom[i].setAttribute("tabindex","-1");
			}
...
function validateGridForSaveDrs(){
	if(!isNewDrs()){
		...
		for(var i=0;i<deliveryTypeDom.length;i++) {
			...
			if(isNull(deliveryTimeMMDom[i].value)){
			...
			if(getCnoteProduct(consgDom[i].value)==getQseries()){
				if(isNull(relationDom[i].value)){
					alert("Please Select Relationship of the receiver with the Consignee at line :"+(i+1));
					relationDom[i].disabled=false;
