 ...
 *
 */
public class CreditCardDrsDAOImpl extends CGBaseDAO implements
CreditCardDrsDAO {

	/** The Constant LOGGER. */
	private final static Logger LOGGER = LoggerFactory
			.getLogger(CreditCardDrsDAOImpl.class);
	/**
...
public	Boolean updateDeliveredDrs(DeliveryDO deliveryDo)throws CGBusinessException, CGSystemException{
		...
		try {
			...
			tx.rollback();
			LOGGER.error("CreditCardDrsDAOImpl::updateDrs ::Exception ",e);
	...
	private void updateDeliveredDetails(DeliveryDetailsDO deliveryDtlsDO,Session session){
		...
		if(!CGCollectionUtils.isEmpty(detailsDOList)){
			...
			detailsDO.setIdProofDO(deliveryDtlsDO.getIdProofDO());
			detailsDO.setRelationDO(deliveryDtlsDO.getRelationDO());
			detailsDO.setIdNumber(deliveryDtlsDO.getIdNumber());
