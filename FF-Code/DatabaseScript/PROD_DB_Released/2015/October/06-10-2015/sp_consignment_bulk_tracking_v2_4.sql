...
-- V2.2 - 1. artf3805555 : Issue in Multiple Tracking CD No column
-- V2.3 - Tracking Enhancement - 1. DRS # and FS/Third Party Name in multiple & bulk tracking modules
--                               2. we need to show the delivery branch name in Multiple & Bulk Tracking.
-- V2.4 - In-Correct Status Reflecting  in Multiple/Bulk Tracking for RTO consgs(Delivery Pending)
--       	- read delivery table for consignment delivery status
-- 		 	- read final weight from consignment table on client request
...
BEGIN
 ...
 ' ,CASE '
  ...
  ' ,CASE '    
    ...
    'THEN '
      '''Delivered'' '
    'WHEN (ffc.EVENT_DATE > ffdd1.TRANS_MODIFIED_DATE_TIME) AND ffc.CONSG_STATUS = ''R'' '
--           'ffdd1.DELIVERY_STATUS = ''O'') '
  ...
  'END '
    ...
    /* Enhancement - Delivery No Added*/
   ' ,ffd.DRS_NUMBER AS DRS_NO '
    ',ffdd1.DELIVERY_TIME '
    'AS DELIVERY_DATE'
    /* Enhancement - Delivery Office Added*/
    ' ,concat( '
        'fdo.OFFICE_CODE '
       ','' - '' ' 
       ',fdo.OFFICE_NAME)'
 ...
 ' ,IF( '
  ...
  ',ffdd1.RECEIVER_NAME) '
    'AS RECEIVER_NAME'
  /* Enhancement - Third Party Name Added*/  
    ' ,CASE'
      ' WHEN ffd.DRS_FOR = ''FS'' '
      'THEN '
 ...
 ' ,CASE '
    ...
    'THEN '    
      'IFNULL(fflc_bag_dox_dsp.TOKEN_NUMBER,fflc_mbag_dox_dsp.TOKEN_NUMBER) '      
    'WHEN fdct.CONSIGNMENT_CODE = ''PPX'' '
    'THEN '    
      'IFNULL(fflc_bag_ppx_dsp.TOKEN_NUMBER,fflc_mbag_ppx_dsp.TOKEN_NUMBER) '    
  'END '
    'AS CD_NO' 
 ' ,CASE '
    'WHEN fdct.CONSIGNMENT_CODE = ''DOX'' '
 ...
 ' ,CASE '
  ...
  ' ,IFNULL(trip1.DEPARTURE_TIME ,fftrp_dsp.DEPARTURE_TIME) AS FLIGHT_DEP'
  ' ,IFNULL(trip1.ARRIVAL_TIME ,fftrp_dsp.ARRIVAL_TIME) AS FLIGHT_ARR'
...
-- 'LEFT JOIN ff_d_consignment_type fdct ON fdct.CONSIGNMENT_TYPE_ID = ffc.CONSG_TYPE '
  ...
  'LEFT JOIN ff_f_load_movement fflm_bag_ppx_dsp '
    ...
    'AND fflm_bag_ppx_dsp.MOVEMENT_DIRECTION = ''D'' '
  ...
  'LEFT JOIN ff_d_vehicle fdv2 ON fdv2.VEHICLE_ID = fflm_bag_dox_dsp.VEHICLE '
  'LEFT JOIN ff_f_transport transport1 ON transport1.VEHICLE = fdv2.VEHICLE_ID '
  ...
  'LEFT JOIN ff_d_office fdo ON fdo.OFFICE_ID = ffd.CREATED_OFFICE_ID '
