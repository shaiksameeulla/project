...
CREATE PROCEDURE `sp_track_consignment`(IN CONSG_NO VARCHAR(25))
   BEGIN
      ...
      SELECT @rownum := @rownum + 1 AS ROW_ID,
        ...
        FROM (-- ROW fOR PICKUP
              ...
              SELECT 'C' AS ARTIFACT_TYPE,
                ...
                FROM ff_f_load_connected fflc
                     ...
                     LEFT JOIN ff_f_trip trip1
                        ON trip1.TRANSPORT = transport1.TRANSPORT_ID
               WHERE ffm.MANIFEST_ID IN
                        (SELECT dispManifest.*
              ...
              SELECT 'C' AS ARTIFACT_TYPE,
                ...
                FROM ff_f_load_connected fflc
                     ...
                     LEFT JOIN ff_f_trip trip1
                        ON trip1.TRANSPORT = transport1.TRANSPORT_ID
               WHERE ffm.MANIFEST_ID IN
                        (SELECT pktOrBagMnfst.*
                           FROM packetOrBagManifest pktOrBagMnfst)
                     AND ffm.MANIFEST_DIRECTION = 'O'
                     AND fflm.MOVEMENT_DIRECTION = 'D'
              UNION
              -- ROW FOR DISPATCH
              ...
              SELECT 'C' AS ARTIFACT_TYPE,
               ...
               WHERE ffm.MANIFEST_ID IN
                        (SELECT pktBagMnfst.*
                           FROM packetOrBagManifestId pktBagMnfst)
                     AND ffm.MANIFEST_DIRECTION = 'I'
                     AND fflm.MOVEMENT_DIRECTION = 'R'
              UNION
              -- ROW FOR RECIEVE
