...
SELECT 'C' AS ARTIFACT_TYPE,
    ...
    ,'status' AS KEY_4
--     ,ffdd.DELIVERY_STATUS AS VALUE_4
    ,if((select count(dd.CONSIGNMENT_ID) from ff_f_delivery_dtls dd where dd.CONSIGNMENT_NUMBER=CONSG_NO) > 0,
       ffdd.DELIVERY_STATUS, (select fp.consg_status from ff_f_consignment fp where fp.consg_no=CONSG_NO)) AS VALUE_4
    ,'reason' AS KEY_5
    ,coalesce(fdr.REASON_NAME,'') AS VALUE_5
    ,'ReceiverName' AS KEY_6
    ,IF(DELIVERY_TYPE = 'O', '( Office Seal & Sign )', ffdd.RECEIVER_NAME) AS VALUE_6      
...
FROM ff_f_delivery ffd 
    ...
    LEFT JOIN ff_d_process fdp ON fdp.PROCESS_CODE = 'DLRS'
    LEFT JOIN ff_d_reason fdr ON ffdd.REASON_ID = fdr.REASON_ID
WHERE ffdd.CONSIGNMENT_NUMBER = if((select count(dd.CONSIGNMENT_ID) from ff_f_delivery_dtls dd where dd.CONSIGNMENT_NUMBER=CONSG_NO) > 0,
                                CONSG_NO,(if((select cn.consg_status from ff_f_consignment cn where cn.consg_no=CONSG_NO)in ('D','S'),
                                (select del.consignment_number from ff_f_delivery_dtls del where del.consignment_number in
                                (select fc.child_consg_number from ff_f_child_consignments fc where fc.consg_id in 
                                                (select fp.consg_id from ff_f_consignment fp where fp.consg_no=CONSG_NO)) limit 1),null)))
