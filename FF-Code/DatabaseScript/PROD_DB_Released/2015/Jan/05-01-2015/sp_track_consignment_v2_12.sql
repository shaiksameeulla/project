...
SELECT ffm.MANIFEST_ID AS PROCESS_ID
       ...
       ,'custName' AS KEY_10
       ,tmp_cnManifested.CUSTOMER AS VALUE_10
			 ,NULL AS KEY_11
       ,NULL AS VALUE_11
       ,NULL AS KEY_12
       ,NULL AS VALUE_12
       ...
       ,NULL AS VALUE_13
       ,'cnStatus' AS KEY_14 
       ,tmp_cnManifested.CONSG_STATUS AS VALUE_14
...
(SELECT ffd.DELIVERY_ID AS PROCESS_ID
      ...
      ,NULL AS OPERATING_LEVEL
      ,fdp.PROCESS_ORDER AS PROCESS_ORDER
      ,IFNULL(ffdd.DELIVERY_TIME, DRS_DATE) AS PROCESS_DATE
      ,NULL AS DIRECTION
      ,'originOff' AS KEY_1
      ...
      ,'other' AS KEY_3,
      CONCAT(ffd.DRS_NUMBER,'#',ffdd.DELIVERY_STATUS,'#',coalesce(fdr.REASON_NAME,'')) AS VALUE_3
      ,'DRSNo' AS KEY_4
      ,ffd.DRS_NUMBER AS VALUE_4
      ,'ReceiverName' AS KEY_5
      ,IF(DELIVERY_TYPE = 'O', '( Office Seal & Sign )', ffdd.RECEIVER_NAME) AS VALUE_5
      ,NULL AS KEY_6
      ,NULL AS VALUE_6
      ,NULL AS KEY_7
      ,NULL AS VALUE_7
...
LEFT JOIN ff_d_reason fdr ON fdr.REASON_ID = ffdd.REASON_ID
...
WHERE ffb.consg_number = CONSG_NO
