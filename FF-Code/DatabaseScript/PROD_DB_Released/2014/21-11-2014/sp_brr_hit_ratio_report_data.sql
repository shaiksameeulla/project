...
--   START TRANSACTION;
  ...
  SELECT
   ...
   ,CASE WHEN fdo_bpcs.CITY_ID = fdimgeo_del_org.CITY_ID THEN 1 ELSE 2 END
      AS 'CATEGORY_ID'
   ...
   ,CASE
      ...
      THEN
        2
      WHEN (fdr.reason_id IS NOT NULL OR
            fdheld.TRANSACTION_NUMBER IS NOT NULL)
      THEN
        7
  ...
  FROM
    ...
    JOIN ff_f_consignment ffc ON ffc.CONSG_ID = ffbtb.CONSG_ID
    ...
    LEFT JOIN ff_d_reason fdr
      ON (fdr.reason_id = ffdd1.reason_id AND
          reason_code = 'MIT')
    /* Held UP information */
    LEFT JOIN ff_f_heldup fdheld
      ON (fdheld.TRANSACTION_NUMBER = ffbtb.CONSG_NO AND
          fdheld.TRANSACTION_TYPE = 'CONG')
  WHERE
    ffbtb.RATE_CALCULATED_FOR = 'B' AND
    /* office serviced by destination pincode should be in manifest office or origin office */
    (bpcs1.OFFICE_ID = ffm_in1.OPERATING_OFFICE OR
     bpcs1.OFFICE_ID = ffbtb.ORG_OFF) AND
...
--   END IF;
  ...
  ON DUPLICATE KEY UPDATE
    CONSIGNMENTS_RECD=VALUES(CONSIGNMENTS_RECD)
   ,CONSIGNMENTS_RET_IN=VALUES(CONSIGNMENTS_RET_IN)
   ,CONSIGNMENTS_ACTUAL=VALUES(CONSIGNMENTS_ACTUAL)
...
--   END IF;
  ...
  ORDER BY
   ...
   ,DATA.CATEGORY_ID
  ON DUPLICATE KEY UPDATE
    CONSIGNMENTS_BOOKED=VALUES(CONSIGNMENTS_BOOKED)
   ,DELIVERED_D1=DELIVERED_D1+VALUES(DELIVERED_D1)
...
--   END IF;
  ...
  SET
   ...
   ,report_data.DELIVERY_OFFICE  = DATA.DELIVERY_OFFICE
   ,report_data.DESTINATION_CITY = DATA.DESTINATION_CITY
   ,report_data.DEL_REGION       = DATA.DEL_REGION
  WHERE
  ...
  -- --------------------------------------------------------------------------------------
  -- STEP 7 - MARK DELIVERED CONSIGNMENTS IN TEMPORARY TABLE AS PROCESSED 'Y' FOR HIT RATIO
  -- --------------------------------------------------------------------------------------
  SELECT
    CURTIME()
  INTO
  ...
  -- --------------------------------------------------------------------------------------
  SET _step = "STEP 7 - MARK DELIVERED CONSIGNMENTS IN TEMPORARY TABLE AS PROCESSED 'Y' FOR HIT RATIO";
  UPDATE
    ff_f_consignment ffc JOIN HITRATIODATA data ON data.CONSG_ID = ffc.CONSG_ID
  SET
    ffc.PROCESSED_FOR_HIT_RATIO = 'Y'
  WHERE
