...
BEGIN
  ...
  START TRANSACTION;
    ...
    FROM (SELECT
        ...
        END AS DISTRIBUTION_CHANNELS1,
        CASE 
        WHEN (bk.SHIPPED_TO_CODE IS NOT NULL AND length(bk.SHIPPED_TO_CODE) > 0) THEN
          bk.SHIPPED_TO_CODE
        WHEN (bk.SHIPPED_TO_CODE IS NULL OR length(bk.SHIPPED_TO_CODE) = 0) THEN
          CASE
          WHEN (bk.CUSTOMER IS NOT NULL AND length(bk.CUSTOMER) > 0) THEN cnew.CUSTOMER_CODE 
          WHEN (bk.CUSTOMER IS NULL OR length(bk.CUSTOMER) = 0) THEN 
            CASE
          ...
          END
        ELSE 'ELSE 1'
        END AS SHIP_TO_CODE, -- ## changed ship to code logic
        'CN' AS SUMMARY_TYPE,
        0 AS UNIT_OF_MEASUREMENT,
        CASE
          WHEN p.CONSG_SERIES = 'T' AND
          (cn.COD_AMT IS NULL OR cn.COD_AMT = 0) AND
          ...
          WHEN p.CONSG_SERIES = 'T' AND
          (cn.COD_AMT IS NOT NULL AND cn.COD_AMT !=0) AND
          ctype.CUSTOMER_TYPE_CODE = 'CR' AND
          cn.CONSG_STATUS != 'S' THEN 'PCDCBD'
          WHEN p.CONSG_SERIES = 'T' AND
          (cn.COD_AMT IS NOT NULL AND cn.COD_AMT !=0) AND
          ctype.CUSTOMER_TYPE_CODE = 'CR' AND
          cn.CONSG_STATUS = 'S' THEN 'PCDCBRD' 
          WHEN p.CONSG_SERIES = 'T' AND
          (cn.COD_AMT IS NULL OR cn.COD_AMT = 0)  AND
          ctype.CUSTOMER_TYPE_CODE in ('BA','BV') THEN 'TPBAB'
          WHEN p.CONSG_SERIES = 'T' AND
          (cn.COD_AMT IS NOT NULL AND cn.COD_AMT !=0) AND
          ctype.CUSTOMER_TYPE_CODE in ('BA','BV') THEN 'PCDBAB'
          ELSE 'OTHERS'
    ...
    `CUSTOMER`,
    `BILLING_STATUS`,
    `CHANGED_AFTER_BILLING_WEIGHT_DEST`,
    `CHANGED_AFTER_NEW_RATE_COMPONENT`,
    `DELTA_TRANSFER`
    )
    ...
    FROM (SELECT
        ...
        WHEN (bk.SHIPPED_TO_CODE IS NULL OR length(bk.SHIPPED_TO_CODE) = 0) THEN
          ...
          END
        ELSE 'ELSE 1'
        END AS CN_SHIP_TO_CODE, -- ## changed ship to code logic
        CASE
          WHEN cnew.DISTRIBUTION_CHANNELS IS NULL THEN '00' ELSE cnew.DISTRIBUTION_CHANNELS
        ...
        cn.VOL_WEIGHT,
        (cn.FINAL_WEIGHT
        -if(ffbc.FINAL_WEIGHT is null, 0, ffbc.FINAL_WEIGHT)) as FINAL_WEIGHT,
        'N' as DT_TO_BRANCH,
        cn.DT_TO_CENTRAL,
        cn.DT_FROM_OPSMAN,
        cn.UPDTAED_PROCESS_FROM,
        'Y' as DT_UPDATE_TO_CENTRAL,
        cn.MOBILE_NO,
        cn.CONTENT,
        cn.PAPER_WORK,
        cn.INSURED_BY,
        ...
        END AS DESTINATION_OFFICE,
        cn.BILLING_STATUS,
        cn.CHANGED_AFTER_BILLING_WEIGHT_DEST,
        cn.CHANGED_AFTER_NEW_RATE_COMPONENT,
        CASE
          WHEN ffbc.CONSG_ID IS NULL THEN 'N'
          ELSE 'Y'
        END AS DELTA_TRANSFER
      FROM ff_f_consignment cn
        -- get customer
        JOIN ff_f_booking bk
          ON bk.CONSG_NUMBER = cn.CONSG_NO
        ...
        LEFT JOIN ff_f_delivery ffd ON ffdd.DELIVERY_ID = ffd.DELIVERY_ID
        LEFT JOIN ff_d_office fdo ON ffd.CREATED_OFFICE_ID = fdo.OFFICE_ID
        -- get billed details of each consignment
        left join (select CONSG_ID, sum(FINAL_WEIGHT) as FINAL_WEIGHT 
                  from ff_f_billing_consignment group by CONSG_ID) ffbc
          on ffbc.CONSG_ID = cn.CONSG_ID
      WHERE cn.BILLING_STATUS in ('RTB', 'RRB') 
    ) RESULT
      JOIN ff_f_billing_consignment_summary cnsum
        ...
        AND cnsum.VERSION = 0)
    WHERE RESULT.CN_SHIP_TO_CODE NOT IN ('ELSE 1', 'ELSE 2');
    -- -------------------------------------------------------------------------------------------------    
    SELECT
      row_count()
    INTO
    ...
    -- implrmrny ifnull condition for change after weight/destination subtraction
    SELECT
      ...
      CASE
        ...
        THEN
          SUM(cr.FINAL_SLAB_RATE - if(bcr.FINAL_SLAB_RATE is not null, bcr.FINAL_SLAB_RATE, 0))
        ELSE
          0
      END
        AS FINAL_SLAB_RATE,
      CASE
        ...
        WHEN cr.RATE_CALCULATED_FOR = 'B' AND
             cn.CHANGED_AFTER_BILLING_WEIGHT_DEST = 'Y'
        THEN
          SUM(cr.FUEL_SURCHARGE - if(bcr.FUEL_SURCHARGE is not null, bcr.FUEL_SURCHARGE, 0))
        ELSE
          0
      END
        AS FUEL_SURCHARGE,
      CASE
        ...
        WHEN cr.RATE_CALCULATED_FOR = 'B' AND
             cn.CHANGED_AFTER_BILLING_WEIGHT_DEST = 'Y'
        THEN
          SUM(cr.RISK_SURCHARGE - if(bcr.RISK_SURCHARGE is not null, bcr.RISK_SURCHARGE, 0))
        ELSE
          0
      END
        AS RISK_SURCHARGE,
      CASE
        ...
        WHEN cr.RATE_CALCULATED_FOR = 'B' AND
             cn.CHANGED_AFTER_BILLING_WEIGHT_DEST = 'Y'
        THEN
          SUM(cr.TO_PAY_CHARGE - if(bcr.TO_PAY_CHARGE is not null, bcr.TO_PAY_CHARGE, 0))
        ELSE
          0
      END
        AS TO_PAY_CHARGE,
      CASE
        ...
        WHEN cr.RATE_CALCULATED_FOR = 'B' AND
             cn.CHANGED_AFTER_BILLING_WEIGHT_DEST = 'Y'
        THEN
          SUM(cr.COD_CHARGES - if(bcr.COD_CHARGES is not null, bcr.COD_CHARGES, 0))
        ELSE
          0
      END
        AS COD_CHARGES,
      CASE
        ...
        WHEN cr.RATE_CALCULATED_FOR = 'B' AND
             cn.CHANGED_AFTER_BILLING_WEIGHT_DEST = 'Y'
        THEN
          SUM(cr.PARCEL_HANDLING_CHARGE - if(bcr.PARCEL_HANDLING_CHARGE is not null, bcr.PARCEL_HANDLING_CHARGE, 0))
        ELSE
          0
      END
      ...
      CASE
        ...
        WHEN cr.RATE_CALCULATED_FOR = 'B' AND
             cn.CHANGED_AFTER_BILLING_WEIGHT_DEST = 'Y'
        THEN
          SUM(cr.AIRPORT_HANDLING_CHARGE - if(bcr.AIRPORT_HANDLING_CHARGE is not null, bcr.AIRPORT_HANDLING_CHARGE, 0))
        ELSE
          0
      END
      ...
      CASE
        ...
        WHEN cr.RATE_CALCULATED_FOR = 'B' AND
             cn.CHANGED_AFTER_BILLING_WEIGHT_DEST = 'Y'
        THEN
          SUM(cr.DOCUMENT_HANDLING_CHARGE - if(bcr.DOCUMENT_HANDLING_CHARGE is not null, bcr.DOCUMENT_HANDLING_CHARGE, 0))
        ELSE
          0
      END
      ...
      CASE
        ...
        WHEN cr.RATE_CALCULATED_FOR = 'B' AND
             ...
             cn.CHANGED_AFTER_NEW_RATE_COMPONENT = 'N'
        THEN
          SUM(cr.OTHER_OR_SPECIAL_CHARGES - if(bcr.OTHER_OR_SPECIAL_CHARGES is not null, bcr.OTHER_OR_SPECIAL_CHARGES, 0))
        WHEN cr.RATE_CALCULATED_FOR = 'B' AND
             cn.CHANGED_AFTER_BILLING_WEIGHT_DEST = 'N' AND
             cn.CHANGED_AFTER_NEW_RATE_COMPONENT = 'Y'
        THEN
          SUM(if(octroi.OTHER_CHARGES IS NULL, 0, octroi.OTHER_CHARGES))
        WHEN cr.RATE_CALCULATED_FOR = 'B' AND
             cn.CHANGED_AFTER_BILLING_WEIGHT_DEST = 'Y' AND
             cn.CHANGED_AFTER_NEW_RATE_COMPONENT = 'Y'
        THEN
      ...
      CASE
        ...
        WHEN cr.RATE_CALCULATED_FOR = 'B' AND
             cn.CHANGED_AFTER_BILLING_WEIGHT_DEST = 'Y'
        THEN
          SUM(cr.SERVICE_TAX - if(bcr.SERVICE_TAX is not null, bcr.SERVICE_TAX, 0))
        ELSE
          0
      END
        AS SERVICE_TAX,
      CASE
        ...
        WHEN cr.RATE_CALCULATED_FOR = 'B' AND
             cn.CHANGED_AFTER_BILLING_WEIGHT_DEST = 'Y'
        THEN
          SUM(cr.EDUCATION_CESS - if(bcr.EDUCATION_CESS is not null, bcr.EDUCATION_CESS, 0))
        ELSE
          0
      END
        AS EDUCATION_CESS,
      CASE
        ...
        WHEN cr.RATE_CALCULATED_FOR = 'B' AND
             cn.CHANGED_AFTER_BILLING_WEIGHT_DEST = 'Y'
        THEN
          SUM(cr.HIGHER_EDUCATION_CES - if(bcr.HIGHER_EDUCATION_CES is not null, bcr.HIGHER_EDUCATION_CES, 0))
        ELSE
          0
      END
      ...
      CASE
        ...
        WHEN cr.RATE_CALCULATED_FOR = 'B' AND
             cn.CHANGED_AFTER_BILLING_WEIGHT_DEST = 'Y'
        THEN
          SUM(cr.STATE_TAX - if(bcr.STATE_TAX is not null, bcr.STATE_TAX, 0))
        ELSE
          0
      END
        AS STATE_TAX,
      CASE
        ...
        WHEN cr.RATE_CALCULATED_FOR = 'B' AND
             cn.CHANGED_AFTER_BILLING_WEIGHT_DEST = 'Y'
        THEN
          SUM(cr.SURCHARGE_ON_STATE_TAX - if(bcr.SURCHARGE_ON_STATE_TAX is not null, bcr.SURCHARGE_ON_STATE_TAX, 0))
        ELSE
          0
      END
      ...
      CASE
        ...
        THEN
          SUM(if(
                octroi.SERVICE_CHARGE IS NULL,
                0,
                octroi.SERVICE_CHARGE)
             -
             if(
                bcr.SERVICE_CHARGE_ON_OCTROI IS NULL,
                0,
                bcr.SERVICE_CHARGE_ON_OCTROI)
             )
        ELSE
          0
      END
      ...
      CASE
        ...
        THEN
          SUM(if(
                octroi.SERVICE_TAX IS NULL,
                0,
                octroi.SERVICE_TAX)
             -
             if(
                bcr.SERVICE_TAX_ON_OCTROI_SERVICE_CHARGE IS NULL,
                0,
      ...
      CASE
        ...
        THEN
          SUM(if(
                octroi.EDUCATION_CESS IS NULL,
                0,
                octroi.EDUCATION_CESS)
             -
             if(
                bcr.EDU_CESS_ON_OCTROI_SERVICE_CHARGE IS NULL,
                0,
      ...
      CASE
        ...
        THEN
          SUM(if(
                octroi.HIGHER_EDUCATION_CESS IS NULL,
                0,
                octroi.HIGHER_EDUCATION_CESS)
             -
             if(
                bcr.HIGHER_EDU_CESS_ON_OCTROI_SERVICE_CHARGE IS NULL,
                0,
      ...
      CASE
        ...
        WHEN cr.RATE_CALCULATED_FOR = 'B' AND
             cn.CHANGED_AFTER_BILLING_WEIGHT_DEST = 'Y'
        THEN
          SUM(cr.GRAND_TOTAL_INCLUDING_TAX - if(bcr.GRAND_TOTAL_INCLUDING_TAX is not null, bcr.GRAND_TOTAL_INCLUDING_TAX, 0))
        ELSE
          0
      END
      ...
      CASE
        ...
        WHEN cr.RATE_CALCULATED_FOR = 'B' AND
             cn.CHANGED_AFTER_BILLING_WEIGHT_DEST = 'Y'
        THEN
          SUM(cr.LC_CHARGE - if(bcr.LC_CHARGE is not null, bcr.LC_CHARGE, 0))
        ELSE
          0
      END
        AS LC_CHARGE,
      CASE
        ...
        WHEN cr.RATE_CALCULATED_FOR = 'B' AND
             cn.CHANGED_AFTER_BILLING_WEIGHT_DEST = 'Y'
        THEN
          (cn.LC_AMT - if(bcr.LC_AMOUNT is not null, bcr.LC_AMOUNT, 0))
        ELSE
          0
      END
        AS LC_AMOUNT,
      ...
      CASE
        ...
        WHEN cr.RATE_CALCULATED_FOR = 'B' AND
             cn.CHANGED_AFTER_BILLING_WEIGHT_DEST = 'Y'
        THEN
          (cn.COD_AMT - if(bcr.COD_AMOUNT is not null, bcr.COD_AMOUNT, 0))
        ELSE
          0
      END
        AS COD_AMOUNT,
    ...
    FROM ff_f_consignment cn
      ...
      LEFT JOIN ff_f_consignment_rate cr
        ON (cr.CONSIGNMENT_ID = cn.CONSG_ID AND cr.BILLED = 'N')
      JOIN ff_f_billing_consignment bc
        ON (bc.CONSG_ID = cr.CONSIGNMENT_ID AND bc.VERSION = 0)
      -- get expenses
      LEFT JOIN (SELECT
                  octraiexpenseentry.CONSG_ID,
                  sum( octraiexpenseentry.HIGHER_EDUCATION_CESS) AS HIGHER_EDUCATION_CESS,
                  sum( octraiexpenseentry.EDUCATION_CESS) AS EDUCATION_CESS,
                  sum( octraiexpenseentry.SERVICE_TAX) AS SERVICE_TAX,
                  sum( octraiexpenseentry.SERVICE_CHARGE) AS SERVICE_CHARGE,
                  sum( octraiexpenseentry.AMOUNT) AS AMOUNT,
                  sum( octraiexpenseentry.OTHER_CHARGES) AS OTHER_CHARGES
                FROM
                  ff_f_expense_entries octraiexpenseentry
                  JOIN ff_f_expense octraiexpense
                    ON (octraiexpense.EXPENSE_ID = octraiexpenseentry.EXPENSE_ID AND
                        octraiexpense.EXPENSE_FOR = 'C')
                  JOIN ff_d_gl_master glm ON (glm.GL_ID = octraiexpense.BANK_GL_ID AND glm.IS_OCTROI_GL = 'Y')
                GROUP BY
                  octraiexpenseentry.CONSG_ID
                ) as octroi
      ON cn.CONSG_ID = octroi.CONSG_ID
