...
drop table if exists ff_f_cn_track;
...
CREATE TABLE ff_f_cn_track( ROW_ID int,
       PROCESS_ID int,
       ARTIFACT_TYPE varchar(10),
	     OPERATING_OFFICE int, -- Added by Saumya
       CONSG_NO varchar(20),
...
select @rownum:=@rownum+1 as ROW_ID,
       ...
       cn_track.ARTIFACT_TYPE,
	     cn_track.OPERATING_OFFICE, -- Added by Saumya
       cn_track.CONSG_NO,
...
SELECT ff_f_manifest.MANIFEST_ID AS PROCESS_ID,
       'M' as ARTIFACT_TYPE,
	     ff_f_manifest.OPERATING_OFFICE as OPERATING_OFFICE, -- Added by Saumya
       CONSG_NO AS CONSG_NO, -- Changed by Saumya
      ...
      convert(ff_f_manifest.DESTINATION_CITY, CHARACTER) as VALUE_8,
        'consgNo' AS KEY_9,
        CONSG_NO as VALUE_9,
        'custName' AS KEY_10, -- version 2.1 for third party manifest
        ...
        NULL AS KEY_13,
        NULL as VALUE_13,
        'cnStatus' AS KEY_14, -- Added in 1.8
...
SELECT  consignment.CONSG_ID AS PROCESS_ID,
       'C' as ARTIFACT_TYPE,
		    consignment.ORG_OFF as OPERATING_OFFICE,  -- Operating Office is considering as ORG_OFF discussed with Saumya
        consignment.CONSG_NO AS CONSG_NO,
        NULL AS MANIFEST_NO,
        -- 'BOOK' as PROCESS_NO,
		    process.PROCESS_CODE as PROCESS_NO, -- modified in 2.3
        NULL AS OPERATING_LEVEL,
...
process.PROCESS_CODE = 'BOOK' -- modified in 2.3
...
SELECT  heldup.HELDUP_ID AS PROCESS_ID,
	      'C' as ARTIFACT_TYPE,
		heldup.OFFICE as OPERATING_OFFICE , -- Added by Saumya
        CONSG_NO AS CONSG_NO,
		NULL AS MANIFEST_NO,
...
SELECT    loadMovement.LOAD_MOVEMENT_ID AS PROCESS_ID,
	       'C' as ARTIFACT_TYPE,
		      loadMovement.OPERATING_OFFICE as OPERATING_OFFICE, -- Added by Saumya
          CONSG_NO AS CONSG_NO,
...
SELECT  delivery.DELIVERY_ID AS PROCESS_ID,
        'C' as ARTIFACT_TYPE,
		    delivery.CREATED_OFFICE_ID as OPERATING_OFFICE,  -- Added By Saumya
        details.CONSIGNMENT_NUMBER AS CONSG_NO,
        NULL AS MANIFEST_NO,
	      p.PROCESS_CODE AS PROCESS_NO,
	      NULL AS OPERATING_LEVEL,
        p.PROCESS_ORDER as PROCESS_ORDER,
        IFNULL(details.DELIVERY_TIME, DRS_DATE) as PROCESS_DATE,       -- Put the delivery time as Process Date
        NULL as DIRECTION,
	      'originOff'AS  	KEY_1,
...
AND details.RECORD_STATUS = 'A' -- ADDED IN V 1.9
...
SELECT booking.BOOKING_ID AS PROCESS_ID,
	     'C' as ARTIFACT_TYPE,
	   booking.BOOKING_OFF as OPERATING_OFFICE,  -- Added by Saumya
...
SELECT  consignment.CONSG_ID AS PROCESS_ID, -- Added at UAT changes in version - 2.3
       'C' as ARTIFACT_TYPE,       
...
-- ######################### Commented as discussed with BA ###############################
-- UPDATE ff_f_cn_track track
-- SET track.OPERATING_LEVEL = 
--     ( CASE
--         -- Check for origin 
--         WHEN EXISTS(select c.ORG_OFF from ff_f_consignment c 
--                 WHERE c.ORG_OFF = track.OPERATING_OFFICE AND c.CONSG_NO = CONSG_NO) 
--                 THEN 10
-- 		-- Check for origin hubs
--         WHEN EXISTS( SELECT ff_d_office.OFFICE_ID
-- 					FROM    (ff_d_office ff_d_office
-- 							INNER JOIN
-- 						 		ff_f_consignment ff_f_consignment
-- 							ON (ff_d_office.OFFICE_ID = ff_f_consignment.ORG_OFF 
-- 								AND ff_f_consignment.CONSG_NO = CONSG_NO))
-- 					INNER JOIN
-- 						ff_d_office_type ff_d_office_type
-- 							ON (ff_d_office.OFFICE_TYPE_ID = ff_d_office_type.OFFICE_TYPE_ID)
-- 					WHERE (ff_d_office_type.OFFICE_TYPE_CODE = 'HO') 
-- 					AND   (ff_d_office.OFFICE_ID = track.OPERATING_OFFICE))
--                 THEN 20
-- 		
-- 		
-- 		-- Check for Destinations
-- 		-- Based on offices that service the destination pin code
-- 		WHEN EXISTS(SELECT ff_d_office.OFFICE_ID
-- 					FROM    (ff_d_office ff_d_office
-- 							INNER JOIN
-- 								ff_f_consignment ff_f_consignment
-- 							ON (ff_d_office.OFFICE_ID IN 
-- 							( select branchPincodeService.OFFICE_ID FROM ff_d_branch_pincode_serviceablity branchPincodeService
-- 								where branchPincodeService.PINCODE_ID = ff_f_consignment.DEST_PINCODE )
-- 								AND ff_f_consignment.CONSG_NO = CONSG_NO))
-- 					INNER JOIN
-- 						ff_d_office_type ff_d_office_type
-- 							ON (ff_d_office.OFFICE_TYPE_ID = ff_d_office_type.OFFICE_TYPE_ID)
-- 					WHERE (ff_d_office_type.OFFICE_TYPE_CODE = 'BO') 
-- 					AND   (ff_d_office.OFFICE_ID = track.OPERATING_OFFICE))
--                 THEN 100
-- 		
-- 		-- Check for Destination Hubs - 80
-- 		-- based on the hubs of the city of the Destination Pin code of the consignments
-- 		WHEN EXISTS(SELECT ff_d_office.OFFICE_ID
-- 					FROM    (ff_d_office ff_d_office
-- 							INNER JOIN
-- 								ff_f_consignment ff_f_consignment
-- 							ON ( ff_d_office.CITY_ID =(select city_id from ff_d_pincode where pincode_id = ff_f_consignment.DEST_PINCODE)
-- 							AND ff_f_consignment.CONSG_NO = CONSG_NO))
-- 					INNER JOIN
-- 						ff_d_office_type ff_d_office_type
-- 							ON (ff_d_office.OFFICE_TYPE_ID = ff_d_office_type.OFFICE_TYPE_ID)
...
-- 					AND   (ff_d_office.OFFICE_ID = track.OPERATING_OFFICE))
--                 THEN 80
-- 		-- Check for Transshipment Destinations - 70
-- 		-- Process
--         ELSE 70 
--     END);
-- the entire data set has to be sorted on Operating level, Process date time, Process Order.
-- Commented by saumya
-- select * from ff_f_cn_track cn_track order by cn_track.OPERATING_LEVEL,cn_track.PROCESS_DATE,cn_track.PROCESS_ORDER;
-- ########################################################
