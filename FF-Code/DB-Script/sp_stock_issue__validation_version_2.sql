DROP PROCEDURE IF EXISTS sp_get_latest_stock_by_leaf_for_issue_2;
CREATE PROCEDURE sp_get_latest_stock_by_leaf_for_issue_2(
   IN leaf                  INT,
   IN office_product_Code   char(20),
   IN itemId                INT)
BEGIN

 DROP TEMPORARY TABLE IF EXISTS TEMP_BASEQUERY;
   CREATE TEMPORARY TABLE BASEQUERY AS
       SELECT 'SA' AS processName,
    ...
    INSERT INTO TEMP_BASEQUERY
     ( SELECT 'SI' AS processName,
      ...
      INSERT INTO TEMP_BASEQUERY
       (SELECT 'SR' AS processName,
         ...
         select * from TEMP_BASEQUERY order by stockdate desc limit 1;
