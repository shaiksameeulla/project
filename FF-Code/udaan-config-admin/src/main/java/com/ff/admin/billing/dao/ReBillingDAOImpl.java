	...
	public List<ReBillConsgAliasTO> getRebillConsignmentData(ReBillingHeaderDO reBillingHeaderDO)throws CGSystemException {
		...
		try {
		    ...
		    reBillAliasTOs = session
			    ...
			    .addScalar("codAmt")
			    .addScalar("lcAmt")
			    .addScalar("serviceOn",Hibernate.STRING)
...
@Override
public Integer checkOldContractAvailability(Integer bookingOfficeId,
		Integer customerId, Date fromDate) throws CGSystemException {

	LOGGER.debug("ReBillingDAOImpl :: checkOldContractAvailability() :: Start --------> ::::::");
	List<BigInteger> contractCount = null;
	Session session = null;
	String query = null;
	Integer count = 0;
	try {
	    session = getHibernateTemplate().getSessionFactory().openSession();
	    query = BillingConstants.GET_OLD_APPLICABLE_CONTRACT_COUNT;
	    contractCount = session
		    .createSQLQuery(query)
		    .setDate("contractValidFromDate", fromDate)
		    .setDate("baValidFromDate", fromDate)
		    .setDate("cashValidFromDate", fromDate)
		    .setInteger("baOffice", bookingOfficeId)
		    .setInteger("cashOffice", bookingOfficeId)
		    .setInteger("customerId", customerId)
		    .list();
	    if(!CGCollectionUtils.isEmpty(contractCount)){
	    	count = contractCount.get(0).intValue();
	    }
	} catch (Exception e) {
	    LOGGER.error("ERROR : BillPrintingDAOImpl.checkOldContractAvailability", e);
	    throw new CGSystemException(e);
	} finally {
	    session.close();
	    session = null;
	...
	LOGGER.debug("ReBillingDAOImpl :: checkOldContractAvailability() :: End --------> ::::::");
	return count;
