...
<script type="text/javascript" src="js/stockmanagement/stockReceipt.js"></script>
...
<div id="wraper"> 
    ...
    <div id="maincontent">
    		<div class="mainbody">
            	<div class="formbox">
	        		<h1><bean:message key="stock.label.ack.title" /></h1>
            	...
            	<div class="formTable">
              		<table border="0" cellpadding="0" cellspacing="7" width="100%">
	                  	<tr>
	            			...
	            			<td width="6%" class="lable"><bean:message key="stock.label.ack.stkIssNum" />:</td>
				            <td width="8%"><html:text property="to.stockIssueNumber" styleId="stockIssueNumber" styleClass="txtbox width145" maxlength="12" size="14" onkeydown="return findOnEnterKey('issue',event.keyCode);" />
				            	<%-- <html:button styleClass="searchButton" property="FIND" alt="Search" styleId="issueSearch" onclick="find('issue');" /> --%>
				            	<html:button styleClass="btnintgrid" property="FIND" alt="Search" styleId="issueSearch" onclick="find('issue');">
				        ...
				        <tr>
				            ...
				            <td width="7%"><html:text property="to.requisitionNumber" styleId="requisitionNumber" styleClass="txtbox width145" maxlength="12" size="14" onkeydown="return findOnEnterKey('req',event.keyCode);" />
				            	<%-- <html:button styleClass="searchButton" property="FIND" alt="Search" styleId="requisitionSearch" onclick="find('req');" /> --%>
				            	<html:button styleClass="btnintgrid" property="FIND" alt="Search" styleId="requisitionSearch" onclick="find('req');" >
				            		<bean:message key="button.search"/>
				            	</html:button>
				            </td>
				            <td width="6%" class="lable"><bean:message key="stock.label.ack.stkackNum" />:</td>
				            <td width="8%"><html:text property="to.acknowledgementNumber" styleId="acknowledgementNumber" styleClass="txtbox width145" maxlength="12" size="14" onkeydown="return findOnEnterKey('ack',event.keyCode);" />
				            	<%-- <html:button styleClass="searchButton" property="FIND" alt="Search" styleId="acknowledgeSearch" onclick="find('ack');" /> --%>
				            	<html:button styleClass="btnintgrid" property="FIND" alt="Search" styleId="acknowledgeSearch" onclick="find('ack');" >
				            		<bean:message key="button.search"/>
				            	</html:button>
				            </td>
				        </tr>
	        		...
	        		<table cellpadding="0" cellspacing="0" border="0" class="display" id="receiptGrid" width="100%">
	                  	<thead>
	           				<tr>
			                    ...
			                    <th width="9%"><bean:message key="stock.label.issue.uom" /></th>
			                    <th width="7%"><bean:message key="stock.label.issue.requestedQty"/></th>
			                     <th width="7%"><bean:message key="stock.label.issue.approvedQty" /></th>
			                    <th width="7%"><bean:message key="stock.label.ack.issueQty" /></th>
			                    <th width="7%"><sup class="star">*</sup><bean:message key="stock.label.ack.receivingQty" /></th>
			                    <%-- <th width="6%"><bean:message key="stock.label.ack.returningQty" /></th> --%>
			                    <th width="9%"><sup class="star">*</sup><bean:message key="stock.label.issue.startNum" /></th>
			                    <th width="12%"><bean:message key="stock.label.issue.endNum" /></th>
			                    <th width="18%"><bean:message key="stock.label.issue.remarks" /></th>
	                    	</tr>
	          			</thead>
	                  	<tbody>
	                  		<c:forEach var="itemDtls" items="${stockReceiptForm.to.receiptItemDtls}" varStatus="loop">
	           				<tr class="${stat.index % 2 == 0 ? 'even' : 'odd'}"><!-- class="gradeA" -->
	                     		...
	                     		<td class="center">
	                   			...
	                   			<td><html:text property="to.rowUom" styleId="rowUom${loop.count}" value="${itemDtls.uom}" styleClass="txtbox width70" tabindex="-1" readonly="true"/></td>
	                   			<td><html:text property="to.rowRequestedQuantity" styleId="rowRequestedQuantity${loop.count}" value="${itemDtls.requestedQuantity}" styleClass="txtbox width55" tabindex="-1" size="11" readonly="true" maxlength="6"/></td>
	                   			...
	                   			<td><html:text property="to.rowIssuedQuantity" styleId="rowIssuedQuantity${loop.count}" value="${itemDtls.issuedQuantity}" styleClass="txtbox width55" tabindex="-1" size="11" readonly="true" maxlength="6" onkeypress="return onlyNumeric(event)" /></td>
	                   			<td><html:text property="to.rowReceivingQuantity" styleId="rowReceivingQuantity${loop.count}" value="${itemDtls.receivedQuantity}" styleClass="txtbox width55" size="11" maxlength="6" onkeypress="return onlyNumeric(event)" onchange="checkBalanceQnty('${loop.count}');seriesValidationWrapper('${loop.count}')" onkeydown = "return enterKeyNav('rowStartSerialNumber${loop.count}' ,event.keyCode);"/></td>
	                   			<td><html:text property="to.rowStartSerialNumber" styleId="rowStartSerialNumber${loop.count}" value="${itemDtls.startSerialNumber}" styleClass="txtbox width110"  onchange="validateSeries(this)" maxlength="12" size="13" onkeydown = "return enterKeyNav('rowRemarks${loop.count}' ,event.keyCode);"/></td>
	                   			<td><html:text property="to.rowEndSerialNumber" styleId="rowEndSerialNumber${loop.count}" value="${itemDtls.endSerialNumber}" styleClass="txtbox width110" tabindex="-1"  readonly="true" maxlength="12" size="13"/></td>
	                   			<td><html:text property="to.rowRemarks" styleId="rowRemarks${loop.count}" value="${itemDtls.remarks}" styleClass="txtbox width145" maxlength="20" onkeydown="enterKeyforStockReceipt(event.keyCode,'${loop.count}','rowRemarks')"/>
	...
	<div class="button_containerform">
		<c:choose>
			<c:when test="${not empty stockReceiptForm.to.stockReceiptId && stockReceiptForm.to.stockReceiptId>0}">
				<%-- <html:button property="Modify" styleClass="btnintform" onclick="save('Update');"><bean:message key="button.modify" /></html:button> --%>
			</c:when>
			<c:otherwise>
  		...
  		<html:button property="Print"  styleId="Print" styleClass="btnintform"><bean:message key="button.print" /></html:button>
