...
function billPrintingStartup(){
	...
	if((OFFICE_TYPE==BR_OFFICE) || (OFFICE_TYPE==HUB_OFFICE)){
			...
			if(OFFICE_TYPE==BR_OFFICE){
			  var branchList=getDomElementById("branchList");
			   var url = './billPrinting.do?submitName=getBills&CustomerId='
				+ custId + "&StartDate=" + startDate + "&EndDate=" + endDate  + "&ProductID="+productId + "&BillingOffs="+branchList[0].value;
...
function printBill(){
	...
	/*	for (var i=0; i<custId.length; i++) {
		...
		if(checkDate()){
			...
			var url = document.getElementById("reportUrl").value + 'pages/reportviewer/salesInvoiceSDSReport.jsp?'
