...
var SELECT_ATLEAST_ONE_ITEM="Please select at least one material details";
...
function getItemDetails(domObj){
	...
	if(issueType!=null){
		...
		if(isNull(getIssuedToTypeDOM().value)&& paymntId!=null){
			...
			domObj.value="";
			setFocusOnStDom(getIssuedToTypeDOM());
			return false;
		}
...
function isSeriesHasBranchProduct(series,rowId,type){
	...
	if(!hasSeries(rowId)){
		...
		clearSeries(rowId);
		setFocusOnStDomId("rowStartSerialNumber"+rowId);
		return false;
	}
...
function hasSeries(rowId){
	...
	var hasSeries=getItemHasSeries(rowId);
	if(hasSeries!=null){
		hasSeries=hasSeries.value;
	}
...
//*****************for series type***********end
 ...
 function setFocusOnStDom(domElemnt){
	 if(domElemnt!=null){
		 setTimeout(function(){domElemnt.focus();},10);
	 }
 }
 function setFocusOnStDomId(domElemntId){
	 var domEmmnt=getDomElementById(domElemntId);
	 if(domEmmnt!=null){
		 setFocusOnStDom(domEmmnt);
	 }
 }
...
//ajaxResponseForSeries :it will be called globlly across stock module
	 ...
	 clearSeries(rowId);
		...
		if (!isNull(ajaxResp)) {
			...
			if(!isNull(result)){
				...
				if(issueScreen!=null && issueTobr !=null && issueTobr.value != issueScreen){
 ...
 function isFutureDateForIssue(selectedDom,domId){
	 ...
	 if(!isNull(dateDom) && isFutureDateByServerDate(dateDom.value,serverDate.value)){
		 dateDom.value="";
		 setFocusOnStDom(dateDom);
		 alert("Date can not be future date");
	 }else{
		 if(!isNull(nextDom)){
			 //nextDom.value="";
			 setFocusOnStDom(nextDom);
		 }
	 }
 }
 /**roundDecimal
	 ...
	 if(enterKeyNavWithoutFocus(keyCode)){
		...
		var  nxtRid=curRcount+1;
		 ...
		 if(!isNull(nextDom)){
			 ...
			 var updateDom=getDomElementById('Update');
			 if(saveDom!=null){
				 setFocusOnStDom(saveDom);
			 }else if(updateDom!=null){
				 setFocusOnStDom(updateDom);
			 }
 ...
 function enterKeyforStockReceipt(keyCode,rowId,domElemetId){
		if(enterKeyNavWithoutFocus(keyCode)){
			...
			if(domElemetId =="rowRemarks"){
				...
				destDomByName=document.getElementsByName("to.rowReceivingQuantity");
			}
 ...
 function isLoggedInOfficeTypeBranch(){
	 var officeType=getDomElementById('officeType');
	 var rhoType=getDomElementById('officeRhoType');
	 var coType=getDomElementById('officeTypeCo');
	 if(officeType!=null && rhoType!=null && coType!=null && !isNull(officeType.value)&& !isNull(rhoType.value) && !isNull(coType.value)){

		 if(officeType.value == rhoType.value || coType.value == officeType.value){
			 alert("This screen is not available for the office Type "+officeType.value);
			 disableAll();
			// getDomElementById("Cancel").disabled=false;
			 enableGlobalButton("Cancel");
		 }
