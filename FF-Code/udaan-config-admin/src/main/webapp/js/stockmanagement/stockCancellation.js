...
function find(type) {
	...
	cancelNo.value=trimString(cancelNo.value);
		if(type == 'stockCancel') {
			if(isNull(cancelNo.value)) {
				alert("Please provide Cancellation Number");
				setFocusOnStDom(cancelNo);
				return false;
			}
...
function validateSeries(domValue){
	...
	//make an ajax call
	if(!isNull(stSeries)){	
		url="./stockIssue.do?submitName=isValidSeriesForIssue&startSerialNum="+domValue.value+"&quantity="+quantity.value+"&issuedTOType="+issuedType+"&itemId="+itemId.value+"&partyTypeId="+officeId+"&itemDetailsId=''"+"&seriesType="+seriesType+"&issuedTO="+getBranchCode().value+"&cityCode="+getCityCode()+"&screenName=cancel";
...
function validateCancelledQnty(){
	...
	qtyDom.value=trimString(qtyDom.value);
	qtyDom.value=parseNumber(qtyDom.value);
	var qty = qtyDom.value;
	if(isNull(qty)){
		alert("Please provide  Quantity");
		qtyDom.value="";
		clearCancelledSeriesDtls();
		setFocusOnStDom(qtyDom);
		return false;
	}
	qty = parseNumber(qty);
