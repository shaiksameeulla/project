...
function addRow() {
	...
	if (transportModeCode == airCode) {
		...
		if ((!isNull(prTO))) {
			...
			if (!isNull(prTO.airlineCode[rowCount - 1])) {
				airlineCode = prTO.airlineCode[rowCount - 1];
			}
				...
				.fnAddData(
						[
								...
								'<input type="text" id="transNumber'
										...
										+ transportNumber
										+ '" size="10" class="txtbox" onkeypress="return alphaNumericAndEnterKeyNav(event,'+rowCount+',\'transNumber\',\'airlineCode\');" onchange="checkTransportNo(this.value,'
										+ rowCount + ');isDuplicateRowExist('+rowCount+', transportModeCode);"/><input type="hidden" id="transportNumber'
										+ rowCount
										+ '" name="to.transportNumber" value = "'
										+ transportNumber + '"/>',
										 '<select name="to.airlineCode" id="airlineCode'+ rowCount +'"  style="width: 120px" class="selectBox" onchange="isDuplicateRowExist('+rowCount+', transportModeCode);" onkeypress = "return enterKeyNav(\'expDepartureTime'+ rowCount +'\',event.keyCode);""><option value="">--Select--</option></select>',
								...
								'<input type="text" id="expArrivalTime'
										...
										+ rowCount
										+ ', transportModeCode)"/><input type="hidden" id="tripNumber'
										+ rowCount
										...
										+ countNo + '"/>' ]);
		if(AIR_LINES_MAP!=null){
		createDropDown('airlineCode'+rowCount, AIR_LINES_MAP);
		}
		getDomElementById('airlineCode'+rowCount).value=airlineCode;
		rowCount++;
		serialNo++;
		countNo++;
		rCount = rowCount;
				...
				.fnAddData(
						[
								...
								'<input type="text" id="transNumber'
										...
										+ gridRowCount +'); isDuplicateRowExist('+gridRowCount+', transportModeCode);"/><input type="hidden" id="transportNumber'
										+ gridRowCount
										+ '" name="to.transportNumber" value = "'
										+ transportNumber + '"/>',
						...
						.fnAddData(
								[
										...
										'<input type="text" id="transNumber'
												...
												+ '" name="transNumber" value = "'
												+ transportNumber
												+ '" class="txtbox" onkeypress="return enterKeyEventNav(event,'+ gridRowCount+',\'transNumber\',\'transportName\');" onblur="checkTransportNo(this.value,'
												+ gridRowCount +'); isDuplicateRowExist('+gridRowCount+', transportModeCode);"/><input type="hidden" id="transportNumber'
												+ gridRowCount
												+ '" name="to.transportNumber" value = "'
												+ transportNumber + '"/>',	
										...
										'<input type="text" id="expDepartureTime'
												...
												+ expDepTime
												+ '" size="10" class="txtbox" onkeypress="return numericColAndEnterKeyNav(event,\'expDepartureTime\',\'expArrivalTime\','+ gridRowCount +');" onblur="checkDepartureTime(this.value,'
												+ gridRowCount + ', transportModeCode)"/>',
										'<input type="text" id="expArrivalTime'
												+ gridRowCount
												+ '" name="to.expArrivalTime"  value = "'
												+ expArrTime
...
function checkTransportNo(transportNo, rowId) {
	...
	if(transportModeCode == trainCode)
		...
		if(!isNull(transportNo) && transportNo.length < 5)
			{
			alert("Please enter 5 digit Train number");
			setTimeout(function() {getDomElementById("transNumber"+rowId).focus();}, 10);
			return;
			}
		}
	...
	if (!isNull(data)) {
		...
		if(responseText!=null){
			...
			if(error!=null){
				...
				//resp =jsonJqueryParser(resp);
				//alert("resp"+resp);
				if(!isNull(resp)){
					//alert("TRANSPORT_ID"+resp["TRANSPORT_ID"]+"TRANSPORTER_ID"+resp["TRANSPORTER_ID"]+"AIRLINE_CODE"+resp["AIRLINE_CODE"]);

					getDomElementById("transportId" + tableRowId).value = resp["TRANSPORT_ID"];
					getDomElementById("transporterId" + tableRowId).value = resp["TRANSPORTER_ID"];
					if (transportModeCode == airCode){ 
						getDomElementById("airlineCode" + tableRowId).value = resp["AIRLINE_CODE"];
					}
...
function savePureRoute() {
	...
	if (checkGridValues() && checkMandatoryForAirlines()) {
		...
		if (check) {
			...
			ajaxCallWithPost(url, "pureRouteForm", saveCallback);
			jQuery.blockUI({ message: '<h3><img src="images/loading_animation.gif"/></h3>' });
		} else {
			jQuery.unblockUI();
...
function searchPureRoute() {
	...
	if (checkFields()) {
		...
		var originRegionId = getDomElementById("originRegion").value;
 ...
 * @returns {Boolean}
 */
function checkDepartureTime(time, rowId, isDuplicateRowExist, transportModeCode) {
	if(!isDuplicateRowExist(rowId, transportModeCode )){
		return false;
	}
	c3 = false;
	if (transportModeCode == airCode) {
 ...
 * @returns {Boolean}
 */
function checkArrivalTime(time, rowId, transportModeCode) {
	if(!isDuplicateRowExist(rowId, transportModeCode)){
		return false;
	}
...
function validateStations(obj,city) {
	...
	if(city == 'origin'){
		document.getElementById("destinationRegion").focus();
	}else{
...
function enterKeyEventNav(e,rowCount,fromField,toField){
	var charCode;
...
function ajaxGetAllAirlines(){
	//alert("ajaxGetAllAirlines");
	//alert(""+AIR_LINES_MAP);
	if(isNull(AIR_LINES_MAP)){
		var urlForAirlines = "./pureRoute.do?submitName=ajaxGetAllAirlinesMap";
		//alert(urlForAirlines);
		ajaxJquery(urlForAirlines, "pureRouteForm", populateAjaxRespForAllAirlines);
	}
}
function populateAjaxRespForAllAirlines(airlinesList){
	//alert("airlinesList"+airlinesList);
	if (!isNull(airlinesList)) {
		AIR_LINES_MAP = airlinesList;
	}else{
		alert("airline  Details are not available");
	}
}

function isDuplicateRowExist(currRowId, transportModeCode){
	if(!isValidDetails(currRowId, transportModeCode)){
		return false;
	}
}
/**
 ...
 * @returns {Boolean}
 */
function isValidDetails(currRowId, transportModeCode){
	if(transportModeCode == 'Air') {
		var tableee = document.getElementById('pureRouteGrid');
		var totalRowCount = tableee.rows.length;
		var transNumberTemp=getValueByElementId("transNumber"+currRowId);
		var airlineCodeTemp=getValueByElementId("airlineCode"+currRowId);
		var expDepartureTimeTemp=getValueByElementId("expDepartureTime"+currRowId);
		var expArrivalTimeTemp=getValueByElementId("expArrivalTime"+currRowId);
		for(var i=1;i<totalRowCount;i++){
			var rowId=i;
			if(currRowId!=rowId){
				var transNumber=getValueByElementId("transNumber"+rowId);
				var airlineCode=getValueByElementId("airlineCode"+rowId);
				var expDepartureTime=getValueByElementId("expDepartureTime"+rowId);
				var expArrivalTime=getValueByElementId("expArrivalTime"+rowId);
				
				if(transNumberTemp == transNumber && airlineCodeTemp ==airlineCode && expDepartureTimeTemp == expDepartureTime && expArrivalTimeTemp == expArrivalTime){
					alert('Duplicate record,already available at line:'+ rowId);
	...
	else if(transportModeCode == 'Road') {
		var tableee = document.getElementById('pureRouteGrid');
		var totalRowCount = tableee.rows.length;
		...
		for(var i=1;i<=totalRowCount;i++){
			...
			if(currRowId!=rowId){
				...
				if(transNumberTemp == transNumber && expDepartureTimeTemp == expDepartureTime && expArrivalTimeTemp == expArrivalTime){
					alert('Duplicate record,already available at line:'+ rowId);
					clearItemDtlsByRowId(currRowId);	
					return false;
...
function clearItemDtlsByRowId(rowId){
	getDomElementById('transNumber'+rowId).value='';
	getDomElementById('expDepartureTime'+rowId).value="";
	getDomElementById('expArrivalTime'+rowId).value="";
	if(AIR_LINES_MAP!=null){
		createDropDown('airlineCode'+rowId, AIR_LINES_MAP);
		}
}

function checkMandatoryForAirlines(){
	var isValid=true;
	var tableee = document.getElementById('pureRouteGrid');
	var totalRowCount = tableee.rows.length;
	var cell = document.getElementsByName('to.airlineCode');
	var transNumber = document.getElementsByName('transNumber');
	if (transportModeCode == airCode){ 
		for(var i=1;i<totalRowCount;i++) {
			if(!isNull(transNumber[i-1].value) && isNull(cell[i-1].value)){
				alert("Please select airline Name at line :"+i);
				isValid=false;
				setFocusOnElmentByDom(cell[i-1]);
				break;
			}
		}
	}
