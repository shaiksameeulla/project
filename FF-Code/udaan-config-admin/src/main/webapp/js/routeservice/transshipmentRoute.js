...
function addTransshipmentRouteRow() {
	if (checkFields()){
		addRow();
		$('#transshipmentRouteGrid').dataTable().fnDraw();
		clearRegionDropDownList("servicedRegionIds" + (rowCount-1), servicedRegionId);
			...
			.fnAddData(
					[
							...
							'<select  id="servicedCityIds'
									...
									+ rowCount
									+ '" name="to.transshipmentNumber" value = ""/><input type="hidden" id="count'
									+ rowCount
...
function saveTransshipmentRoute() {
	...
	if (checkGridValues()) {
		...
		if (check) {
			...
			ajaxCallWithPost(url, "transshipmentRouteForm", saveCallback);
			jQuery.blockUI({ message: '<h3><img src="images/loading_animation.gif"/></h3>' });
		} else {
			jQuery.unblockUI();
...
function getStationByRegion(region) {
	...
	if (!isNull(region)) {
		var url = "./transshipmentRoute.do?submitName=getStationsByRegion&regionId="
				+ region;
		ajaxCallWithPost(url, "transshipmentRouteForm", populateCity);
	} else {
...
function getGridStationByRegion(region, rowId) {
	...
	if (!isNull(region)) {
		...
		if (check) {
			var url = "./transshipmentRoute.do?submitName=getStationsByRegion&regionId="
					+ region;
			ajaxCallWithPost(url, "transshipmentRouteForm", populateGridCity);
		}
	} else {
...
function searchTransshipmentRoute() {
	if (checkFields()) {
		...
		ajaxCallWithPost(url, "transshipmentRouteForm",
...
function addTransshipementDataRow(rNum){
	
		var transshipmentNumber = "";
		var servicedRegionId = "";
		var servicedCityId = "";
		var servicedCityList = "";

		if (!isNull(trTO)) {
			if ((!isNull(trTO[rNum - 1]))) {
				if (!isNull(trTO[rNum - 1].transshipmentRouteId))
					transshipmentNumber = trTO[rNum - 1].transshipmentRouteId;
				if (!isNull(trTO[rNum - 1].servicedRegionId))
					servicedRegionId = trTO[rNum - 1].servicedRegionId;
				if (!isNull(trTO[rNum - 1].servicedCityId))
					servicedCityId = trTO[rNum - 1].servicedCityId;
				if (!isNull(trTO[rNum - 1].servicedCityList))
					servicedCityList = trTO[rNum - 1].servicedCityList;
			}
		}
		...
		document.getElementById("transshipmentNumber"+rNum).value = transshipmentNumber;
		clearRegionDropDownList("servicedRegionIds" + rNum, servicedRegionId);
		loadServicedCityDropDown("servicedCityIds" + rNum, servicedCityList,
				servicedCityId);
...
function clearGrid() {
	...
	//if (rowCount > 1) {
		...
		$('#transshipmentRouteGrid').dataTable().fnClearTable();
		rowCount = 1;
		serialNo = 1;
	//}
}
