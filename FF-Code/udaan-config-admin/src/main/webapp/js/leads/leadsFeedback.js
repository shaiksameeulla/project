...
function addleadPlanningRow() {
	...
	'<input type="text" id="purposeOfVisitors'+ rowCount +'" name="to.purposeOfVisitors" size="20" maxlength="50" class="txtbox" onkeypress="return charactersForPurVisit(event);" />',
	...
	'<input type="text" id="timeStr'+ rowCount +'" name="to.timeStr" size="10" maxlength="5" class="txtbox" onkeypress="return allowOnlyNumbers(event);"/>',
...
function saveOrUpdateLeadsFeedbackDtls(){
	...
	if(confirm("Do you want to Save the feedback Details")){
...
function callBack(data){
	if(data != null){
		...
		if(viewUpdateFeedbackTO.transMag!="LM034"){
			alert('Feedback saved successfully.\n');
			buttonDisabled("save","btnintform");
			jQuery("#" + "save").addClass("btnintformbigdis");
			//window.location.reload();
		}else{
			alert('Feedback could not be saved successfully.');
		}
	}
}
...
function validateMandatoryRowFields(rowId){
	...
	if(isNull(dateStr.val())){
			alert("Please enter Date of Visit at row no." +rowId);
			$("#dateStr"+ rowId).focus();
			return false;
	...
	if(isNull(purposeOfVisitors.val())){
		...
		alert("Please enter purpose of visit at row no." +rowId);
...
function populatetLeadPlansDetails(planDetals){
	if(!isNull(planDetals)){
		if(isJsonResponce(planDetals)){
			return ;
		}
...
function disableGridDtls(planTOArray,rowId){
	...
	if(!isNull(planTOArray.feedbackCode)){
		...
		disableFieldById("timeStr"+ rowId);
