...
function getAllOffices(domElement) {
	if (!isNull(domElement.value)) {
		...
		var url = "./validateExpense.do?submitName=getAllOfficesByCity&cityId="
				+ cityId;
...
function validatePopup(rowId) {
	...
	var url = "./expenseEntry.do?submitName=searchForValidateExpense"
			+ "&txNumber=" + txNo + "&expenseFor=" + expFor + "&officeId="
			+ officeId;
	if (!isNull(url)) {
		showProcessing();
		var popup = window
				.open(
						url,
						'ValidateExpense',
						'scrollbars=no,toolbar=no,menubar=no,location=no,directories=no,status=no,hight='
								+ screen.height
								+ ',width='
								+ screen.width
								+ ',resizable=no,fullscreen=no');
		popup.moveTo(0, 0);
		popup.onbeforeunload = function(){hideProcessing();};
	}
}

/**
 * validate the TxNo. format i.e. Office Code + Tx. Code + 6 digit + "-1" (MUMB
 * EX 123456 / MUMB EX 123456-1)
 * 
 * @param domElement
 * @returns {Boolean}
 */
		...
		if (domValue.length == 12 || domValue.length == 14) {
			...
			if (officeCodePattern.test(domValue.toUpperCase().substring(0, 4))
					&& domValue.substring(4, 6).toUpperCase() == txCode
							.toUpperCase()
					&& (numpattern.test(domValue.substring(6)) || (numpattern
							.test(domValue.substring(6, 12)) && domValue
							.substring(12) == "-1"))) {
				if (domValue.toUpperCase().substring(0, 4) == selectedOffCode) {
					return true;
				} else {
					alert("Transaction number does not belong to selected office:"
							+ selectedOffCode);
					domElement.value = "";
					setTimeout(function() {
						domElement.focus();
