...
$(document).ready(function() {
	var oTable = $('#heldUpGrid').dataTable( {         
	"sScrollY": "70",         
	"sScrollX": "100%",         
	"sScrollXInner": "140%",
	"bInfo": false,
 ...
 * @author narmdr
 */
function validateTransactionNumber(evt, transactionNumberObj){
	var charCode;
	if (window.event) {
	...
	transactionNumberObj.value = transactionNumberObj.value.toUpperCase();
	if (charCode == 13) {
		...
		/*
		 * if(transactionTypeObj instanceof $) { alert(' transactionTypeObj is a
		 * jQuery object! '); } if(txTypeObj instanceof $) { alert(' txTypeObj
		 * is a jQuery object! '); } if ( txTypeObj.jquery ) { // falsy, since
		 * it's undefined alert(' txTypeObj is a jQuery object! '); }
		 * 
		 * if ( transactionTypeObj.jquery ) { // truthy, since it's a string
		 * alert(' transactionTypeObj is a jQuery object! '); } return;
		 */

		if (transactionTypeObj.val() == $("#transactionTypeConsignment").val()) {
			if (!isValidConsgNo(transactionNumberObj)) {
				enterKeyNavigationFocus(evt, 'transactionNumber');
			} else {
				enterKeyNavigationFocus(evt, 'reasonName');
			}
		}
		if (transactionTypeObj.val() == $("#transactionTypeOpenManifest").val()
				|| transactionTypeObj.val() == $("#transactionTypeOGM").val()) {
			if (!isValidPacketNo(transactionNumberObj)) {
				enterKeyNavigationFocus(evt, 'transactionNumber');
			} else {
				enterKeyNavigationFocus(evt, 'reasonName');
			}
		...
		if (transactionTypeObj.val() == $("#transactionTypeBplDox").val()
				|| transactionTypeObj.val() == $("#transactionTypeBplParcel")
						.val()) {
			if (!isValidBplNo(transactionNumberObj)) {
				enterKeyNavigationFocus(evt, 'transactionNumber');
			} else {
				enterKeyNavigationFocus(evt, 'reasonName');
			}
		...
		if (transactionTypeObj.val() == $("#transactionTypeMbpl").val()) {
			if (!isValidMbplNo(transactionNumberObj)) {
				enterKeyNavigationFocus(evt, 'transactionNumber');
			} else {
				enterKeyNavigationFocus(evt, 'reasonName');
			}
		...
		if (transactionTypeObj.val() == $("#transactionTypeMblDispatch").val()) {
			if (!isValidGatepassNo(transactionNumberObj)) {
				enterKeyNavigationFocus(evt, 'transactionNumber');
			} else {
				enterKeyNavigationFocus(evt, 'reasonName');
			}
		...
		if (transactionTypeObj.val() == $("#transactionTypeAwbCdRr").val()) {
			if (!isValidAwbCdRrNumber(transactionNumberObj)) {
				enterKeyNavigationFocus(evt, 'transactionNumber');
			} else {
				enterKeyNavigationFocus(evt, 'reasonName');
			}
...
function validateMandatoryFields(){
	...
	var currentLocationObj = $("#currentLocation");
	...
	if(isNull(currentLocationObj.val())){
		focusAlertMsg4Select(currentLocationObj, "Current Location");
		return false;
	}
...
function isValidConsgNo(consgNoObj) {
	...
	if (!consgNoObj.value.substring(0, 1).match(letters)
			|| !consgNoObj.value.substring(4, 5).match(alphaNumeric)
			|| !consgNoObj.value.substring(1, 4).match(numpattern)
			|| !numpattern.test(consgNoObj.value.substring(5))) {
		clearFocusAlertMsg(consgNoObj, "Consignment No. Format is not correct!");
		return false;
	}
...
function validateFieldLength(transactionNumberObj){
	var transactionTypeObj = $("#transactionType");
	if(transactionTypeObj.val()==$("#transactionTypeConsignment").val()){
		document.getElementById("transactionNumber").maxLength = 12;
	}
	if(transactionTypeObj.val()==$("#transactionTypeOpenManifest").val() ||
			transactionTypeObj.val()==$("#transactionTypeOGM").val()){
		document.getElementById("transactionNumber").maxLength = 10;
	}
	if(transactionTypeObj.val()==$("#transactionTypeBplDox").val() ||
...
function validateTransactionNumberMouseEvent(evt, transactionNumberObj) {
	var charCode;
	if (window.event) {
		charCode = window.event.keyCode; // IE
	} else {
