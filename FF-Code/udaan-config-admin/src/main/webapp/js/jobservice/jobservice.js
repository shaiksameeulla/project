...
function addRow(record) {
	...
	if(isNull(record.failureFile)){
		...
		failureFile = '<a href="./jobService.do?submitName=downloadJobResponseFile&jobNumber='+record.jobNumber+'&fileType=F">Download</a>';
	}
	if(isNull(record.successFile)){
		successFile = '-';
	}else{
		successFile = '<a href="./jobService.do?submitName=downloadJobResponseFile&jobNumber='+record.jobNumber+'&fileType=S">Download</a>';
	}
		...
		$('#example')
				...
				.fnAddData(
						[
						 	...
						 	'<span id="fileSubmissionDate">'+record.fileSubDate+'</span>',
						 	'<span id="remarks">'+record.remarks+'</span>',
						 	'<span id="percentageCompleted">'+record.percentageCompleted+'</span>',
						 	'<span id="jobStatus">'+getStatus(record.jobStatus)+'</span>',
...
function getJobServices(){
	...
	//if(validateDates()){
		var url = "./jobService.do?submitName=getJobServices&jobNumber="+jobNumber+"&processCode="+processCode+"&fromDate="+fromDate+"&toDate="+toDate;
...
function getJob(){
	...
	if(!isNull(getDomElementById("jobNumber"))){
		jobNumber = getDomElementById("jobNumber").value;
		var url = "./jobService.do?submitName=getJobServices&jobNumber="+jobNumber;
...
function assignJobDetails(data){
	if(!isNull(data) && data.length > 0){
		...
		if(isNull(record.failureFile)){
			...
			getDomElementById("errorFile").innerHTML = '<a href="./jobService.do?submitName=downloadJobResponseFile&jobNumber='+record.jobNumber+'&fileType=F">Download</a>';
		}
		
		if(isNull(record.successFile)){
			getDomElementById("successFile").innerHTML = 'Not available';
		}else{
			getDomElementById("successFile").innerHTML = '<a href="./jobService.do?submitName=downloadJobResponseFile&jobNumber='+record.jobNumber+'&fileType=S">Download</a>';
		}
