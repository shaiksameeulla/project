...
function printReport(){	
	...
	var loadName = document.getElementById('loadId').options[document.getElementById('loadId').selectedIndex].text;
	    ...
	    /*  var customer = document.getElementById('bussinessName').value;
			  if(customer =='' || customer==null || customer=="0"){
			          alert('Please select CustomerName');
					  return false;
               }  */
			  
	     var productId = document.getElementById('loadId').value;
	...
	//alert("Load id : "+loadId);
	
	/* alert("Origin hub :"+hub);
	alert("Origin City:"+originCity); */
	 /* alert("End Date  :"+endDate); */ 
			  ...
			  var url = '/udaan-report/ud/pages/reportveiwer/vendorRTORunSheetViewer.jsp?'	
				 + "startDate="+startDate
				 + "&endDate="+endDate
...
<a href="/udaan-web/login.do?submitName=silentLoginToApp&screenCode=<c:out value='${menuNode.applScreenDO.screenCode}'/>&moduleName=<c:out value='${menuNode.applScreenDO.moduleName}'/>&appName=udaan-config-admin" target="_blank"><c:out value='${menuNode.applScreenDO.screenName}'/></a>
    ...
    <div class="clear"></div>
     <!-- main content -->
      <div id="maincontent">
        <div class="mainbody">
          <div class="formbox">
           <font size="3"><center><h1> RTO-COD DRS Report</h1></center></font>
   ...
   <form:form name="frm" method="post" action="vendorRTOReport.htm" commandName="branchDO" onsubmit="return validate();">
          ...
          <td width="24%" class="lable"><span class="mandatoryf">*</span> Origin Region</td>
			...
			<c:choose>
               <c:when test="${officeId eq  '3' && regionList.size()==1 }">
				<form:select path="region"  class="selectBox width130"   disabled="true">
