...
function getOffices(cityId,cityName){
	...
	var branchCode;
	$.getJSON("getOfficesByCityIdForHub.htm", {
		cityId : cityId,
...
function getOfficesForALLOriginBranchs(officeCode){
	...
	if(officeCode=='ALL'){
		var branchCode= new Array();
		$.getJSON("getOfficesForALLOriginBranchs.htm", {
...
function printReport(){	
	...
	var originCity=$("#city2" ).val();
	var branch=$("#officeCodeValue" ).val(); 
    ...
    var originCityName = document.getElementById('city2').options[document.getElementById('city2').selectedIndex].text;
	var originBranchName = document.getElementById('officeCodeValue').options[document.getElementById('officeCodeValue').selectedIndex].text;
	var destRegionName = document.getElementById('destOrigin').options[document.getElementById('destOrigin').selectedIndex].text;
	var destStationName = document.getElementById('city1').options[document.getElementById('city1').selectedIndex].text;
	var destBranchName = document.getElementById('officeCode').options[document.getElementById('officeCode').selectedIndex].text;
	...
	var typeName = document.getElementById('typeId').options[document.getElementById('typeId').selectedIndex].text;
	      ...
	      var originStation = document.getElementById('city2').value; 
			  if(originStation =="0" || originStation =='' || originStation==null){
			          alert('Please select Origin Station');
					  return false;
               } 
	     var originBranch = document.getElementById('officeCodeValue').value;
		...
		var pendingId = document.getElementById('pendingId').value;
			  if(pendingId =='' || pendingId==null || pendingId=="0"){
			          alert('Please select Pending');
					  return false;
               }
	     ...
	     var date = document.getElementById('fromDate').value;
			 ...
			 /* alert("originRegion:"+originRegion);
			  ...
			  alert("destRegion:"+destRegion1);
			  alert("destRegionName:"+destRegionName);
			  alert("destStation:"+destStation); 
			  alert("destStationName:"+destStationName);
			  alert("destBranch:"+destBranch); 
			  alert("destBranchName5555:"+destBranchName);
			  alert("typeId:"+typeId);
	         ...
	         alert("date:"+date); */
			  ...
			  var url = '/udaan-report/ud/pages/reportveiwer/DestinationHubToDispbranchRunSheetViewer.jsp?'	
				 + "date="+Date  
...
<a href="/udaan-web/login.do?submitName=silentLoginToApp&screenCode=<c:out value='${menuNode.applScreenDO.screenCode}'/>&moduleName=<c:out value='${menuNode.applScreenDO.moduleName}'/>&appName=udaan-config-admin" target="_blank"><c:out value='${menuNode.applScreenDO.screenName}'/></a>
    ...
    <div class="clear"></div>
     <!-- main content -->
      <div id="maincontent">
        <div class="mainbody">
          <div class="formbox">
           <font size="3"><center><h1>Destination Hub To Dispatch Branch Report</h1></center></font>
   ...
   <form:form name="frm" method="post" action="destinationHubToDispatchBranch.htm" commandName="branchDO" >
      ...
      <div class="formTable">
        <table border="0" cellpadding="0" cellspacing="3" width="100%">
		  ...
		  <c:choose>
		    ...
		    <td width="24%" class="lable"><span class="mandatoryf">*</span> Origin Region</td>
			 ...
			 <td width="17%">
			  ...
			  <td width="24%" class="lable"><span class="mandatoryf">*</span> Origin Station</td>
	              <td width="17%">
	            <form:select path="cityId"  id="city2" class="selectBox width130" onchange="getOfficesForDestination(this.value)">
					<option value="0" >Select</option>
					<!-- <option value="1110000" >All</option> -->
					<form:options items="${DestcityList}" itemValue="cityId" itemLabel="cityName"/>
				</form:select>
			</td>
			  <td width="24%" class="lable"><span class="mandatoryf">*</span>Origin Branch</td>
			     <td width="17%">
	           <form:select path="" id="officeCodeValue" class="selectBox width130">
             		<option value="0">Select</option>
             		<option value="ALL">ALL</option>
             		<option value="${officeCodeValue}">ALL</option>
             	</form:select>
			   </td>
		  ...
		  <tr>
		   <td width="24%" class="lable"><span class="mandatoryf">*</span> Origin Region</td>
			 <td width="17%">
	          <form:select path="region" class="selectBox width130">
					<option value="0" >Select</option>
					<option value="ALL" >ALL</option>
				</form:select>
			</td>
			...
			<td width="24%" class="lable"><span class="mandatoryf">*</span> Origin Station</td>
			 <td width="17%">
	          <form:select path="cityId" id="city2" class="selectBox width130" onchange="getOfficesForALLOriginBranchs(this.value,2)">
					<option value="0" >Select</option>
					<option value="ALL" >ALL</option>
				</form:select>
			</td>
		 ...
		 <tr>
		   ...
		   <td width="24%" class="lable"> <span class="mandatoryf">*</span> Type </td>
			  <td width="17%"> 
			  <select  id="typeId" property="typeId" class="selectBox width130"  onchange="validate();"> 
            ...
            <td width="17%">
				  <form:select path="productSerices" id="productId" class="selectBox width130" multiple="multiple">
             		<option value="0">Select</option>
             		<option value="16">ALL</option>
             		<form:options items="${productList}" itemLabel="productSerices"/>
             	</form:select>
             </td>
  
			   <td width="24%" class="lable"> <span class="mandatoryf">*</span> Pending Details </td>
              <td width="17%">  
                <select  id="pendingId" property="PendingId" class="selectBox width130"  onchange="validate();"> 
				 <option value="" >Select</option>
		    ...
		    <tr>
								<td class="lable"><sup class="star">* </sup>Date<bean:message/> </td>
