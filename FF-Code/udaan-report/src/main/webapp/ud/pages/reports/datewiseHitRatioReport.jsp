...
<script>
	...
	function calculateDateDiff(fromDate, toDate) {
		...
		var _fromDateObj = new Date(_fromDate[2], _fromDate[1] - 1,
				_fromDate[0], 00);
	...
	function printReport() {
		...
		var originRegionName = document.getElementById('region').options[document
				.getElementById('region').selectedIndex].text;
		var originCityName = document.getElementById('city1').options[document
				.getElementById('city1').selectedIndex].text;
		var originBranchName = document.getElementById('officeCode').options[document
				.getElementById('officeCode').selectedIndex].text;

		var customerName = document.getElementById('bussinessName').options[document
				.getElementById('bussinessName').selectedIndex].text;
		var destRegionName = document.getElementById('destOrigin').options[document
				.getElementById('destOrigin').selectedIndex].text;
		var destinCityName = document.getElementById('city2').options[document
				.getElementById('city2').selectedIndex].text;
		var financialProdName = document.getElementById('FinaProductId').options[document
		        .getElementById('FinaProductId').selectedIndex].text;  
		

		var originRegion1 = document.getElementById('region').value;
		if (originRegion1 == "0" || originRegion1 == ''
				|| originRegion1 == null) {
		...
		if (originStation == "0" || originStation == ''
				|| originStation == null) {
		...
		if (bussinessName == '' || bussinessName == null
				|| bussinessName == "0") {
		...
		if (noOfDays > 29) {
			alert("User Restriction:Date Range should not be greated than 30 days:");
		...
		var len = productId1.length;
		if (len > 0) {
			productCode.push(productId1);
		} else {
		...
		var url = '/udaan-report/ud/pages/reportveiwer/datewiseHitRatioReportViewer.jsp?'
				+ "DestCity1="
				+ DestCity
				+ "&StartDate1="
				+ StartDate
	...
	<div id="wraper">
		...
		<div id="maincontent">
			<div class="mainbody">
				<div class="formbox">
					<font size="3"><center>
							<h1> Customer Wise Hit Ratio Report </h1>
						</center></font>
				...
				<form:form name="frm" method="post"
					action="datewiseHitRatioReport.htm" commandName="branchDO">

					<div class="formTable">
						<table border="0" cellpadding="0" cellspacing="3" width="100%">

							<tr>
								<td width="12%" class="lable"><span class="mandatoryf">*
								</span>Origin Region</td>
								...
								<c:when test="${regionList.size()==1}">
									<form:select path="region" id="region"
										class="selectBox width300"
										onchange="getCitiesByRegCode(this.value,1)" disabled="true">
										<!--   <option value="0">Select</option> -->
										<form:options items="${regionList}" itemValue="regionCode"
											itemLabel="regionName" />
									</form:select>
								...
								<td width="12%" class="lable"><span class="mandatoryf">*</span>Origin
									Station</td>
								<td width="17%"><c:choose>
										<c:when test="${regionList.size()==1 && cityDTO.size()>1 }">
											<form:select path="cityId" id="city1"
												class="selectBox width300" onchange="getOffices(this.value)">
												<option value="0">Select</option>
												<form:options items="${cityDTO}" itemValue="cityId"
													itemLabel="cityName" />
											</form:select></td>
								</c:when>
			 ...
			 --%>
							...
							<tr>
								...
								<c:choose>
									<c:when test="${offices.size() > 1 && cityList.size() == 1}">
										<td width="12%" class="lable"><span class="mandatoryf">*</span>Origin
											Branch</td>
										<td width="17%" align="left"><form:select
												path="branchCode" id="officeCode" class="selectBox width300">
												<option value="ALL">select</option>
												<form:options items="${offices}" itemValue="officeCode"
								...
								<td class="lable"><sup class="star">*</sup>Financial
									Product</td>
								<td width="17%" align="left"><form:select
										path="productSerices" id="FinaProductId"
										class="selectBox width300"
										onchange="getCustomerByProductId(this.value,document.getElementById('officeCode').value)">
										<option value="0">Select</option>
										<form:options items="${finacialProductList}"
											itemValue="productId" itemLabel="productName" />
										<%-- <option value="ALL">ALL</option>  --%>
									</form:select></td>
				    ...
				    <option value="ALL">ALL</option> 
							...
							<tr>
								<td width="12%" class="lable"><span class="mandatoryf">*</span>Destination
									Region</td>
								<td width="17%"><c:choose>
										<c:when test="${officeId eq  '3'}">
											<form:select path="region" id="destOrigin"
												class="selectBox width300"
												onchange="getCitiesByRegCodeFindOriginSta(this.value,2)">
												<option value="0">Select</option>
								...
								<c:otherwise>
									<form:select path="region" id="destOrigin"
										class="selectBox width300"
										onchange="getCitiesByRegCode1(this.value,2)">
										<option value="0">Select</option>
										<option value="1000">ALL</option>
										<form:options items="${regionList}" itemValue="regionCode"
											itemLabel="regionName" />
								...
								<td width="12%" class="lable"><span class="mandatoryf">*</span>Destination
									Station</td>
								<td width="17%"><form:select path="cityId" id="city2"
										class="selectBox width300">
										<option value="0">Select</option>
									</form:select></td>
								<td colspan="1"><br /> <br /></td>
							...
							<tr>
								...
								<td align="left">&nbsp;<input type="text"
									name="to.fromDate" style="height: 20px" value="" id="fromDate"
					...
					<c:choose>				
					 <c:when test="${officeId eq '3'}">		    
                       <form:select path="customerId" id="bussinessName"  class="selectBox width130">
             		     <option value="0">Select</option>
             		   <form:options items="${customerOffice}" itemValue="customerId"	itemLabel="bussinessName" />
             	     </form:select>  </td> 
             	    </c:when>
             	 <c:when test="${officeId eq '5'}">		    
                 <form:select path="customerId" id="bussinessName"  class="selectBox width130">
             		<option value="0">Select</option>
             		<form:options items="${customerOffice}" itemValue="customerId"
													itemLabel="bussinessName" />
             	</form:select>  </td> 
             	</c:when>
               		<c:otherwise>
               		<form:select path="customerId" id="bussinessName"  class="selectBox width130">
             		...
             		<option value="ALL" >ALL</option>
								...
								<td width="17%"><form:select path="customerId"
										id="bussinessName" class="selectBox width300">
										<option value="0">Select</option>
									</form:select></td>

								<%-- <td class="lable"><span class="mandatoryf">*</span> Product</td>
								<td width="17%"><form:select path="productSerices"
										id="productId" itemValue="productId"
										class="selectBox width300" multiple="multiple" size="8"
										onchange="getCustomerNameFromProductId(this.value)">
										<option value="0">Select</option>
								          <option value="16">ALL</option>
										<form:options items="${productList}"
											itemLabel="productSerices" />
									</form:select></td> --%>

								<td class="lable"> <span class="mandatoryf">*</span> Product</td>	
            <td width="17%">
				  <form:select path="productSerices" id="productId" class="selectBox width300" multiple="multiple" size="8">
             		<option value="0">Select</option>
