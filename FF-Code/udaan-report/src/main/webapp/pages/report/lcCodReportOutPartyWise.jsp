		...
		<div id="wraper">
			...
			<div id="maincontent">
				<div class="mainbody">
					<div class="formbox">
						<h1>
							<bean:message key="label.lcCodReport.printHeaderPartyWise" />
						</h1>
						<div class="mandatoryMsgf">
							<span class="mandatoryf"><bean:message
									key="symbol.common.star" /></span> &nbsp;
					...
					<table border="0" cellpadding="0" cellspacing="3" width="100%">
						<tr>
							<td width="10%" class="lable"><sup class="star">* </sup> <bean:message
									key="label.lcCodReport.type" /></td>
							<td width="16%"><html:select property="to.type"
									styleId="typeList" styleClass="selectBox width130">
									<html:option value="">
										<bean:message key="label.common.select" />
									</html:option>
									<html:option value="ALL">
										<bean:message key="label.lcCodReport.all" />
									</html:option>
							...
							<td width="25%"><html:select property="to.productTO"
									styleId="productTo" styleClass="selectBox width140"
									onchange="clearFieldsOnProduct();">
									<html:option value="">
										<bean:message key="label.common.select" />
									</html:option>
									<html:option value="D,L">
										<bean:message key="label.lcCodReport.all" />
									</html:option>
							...
							<td width="10%" class="lable"><sup class="star">* </sup> <bean:message
									key="label.lcCodReport.sortingOrder" /></td>
							<td width="16%"><html:select property="to.sortOrder"
									styleId="sortOrderList" styleClass="selectBox width130">
									<c:forEach var="type" items="${sortingOrderTo}"
										varStatus="loop">
										<html:option value="${type.stdTypeCode } ">
											<c:out value="${type.description } " />
										</html:option>
						...
						<tr>
							<td width="10%" class="lable"><div id="sortingLabelList">
									<sup class="star">* </sup>
									<bean:message key="label.lcCodReport.sorting" />
								</div></td>
							<td width="16%"><html:select property="to.sorting"
									styleId="sortingList" styleClass="selectBox width130">
									<html:option value="">
										<bean:message key="label.common.select" />
									</html:option>
									<c:forEach var="type" items="${sortingTo}" varStatus="loop">
										<c:if test="${type.stdTypeCode ne 'Disabled' }">
											<html:option value="${type.stdTypeCode } ">
												<c:out value="${type.description } " />
											</html:option>
										</c:if>
							...
							<td><html:text property="to.startDate"
							...
							<td><html:text property="to.endDate"
						...
						<tr>
							...
							<td width="10%" class="lable"><sup class="star">* </sup> <bean:message
									key="label.lcCodReport.region" /></td>
							<td width="16%">
								<%-- <html:select property="to.regionTO" styleId="regionList" 
								...
								name="regionList" id="regionList" class="selectBox width200"
								onchange="getCustomerByRegionAndProduct();">
				...
				<select name="lcCodTO.regionTo" id="regionList"
					...
					<c:forEach var="regions" items="${wrapperReportTO.getRegionTO()}">
						<option value="${regions.regionId }">${regions.regionName}</option>
							...
							<td width="10%" class="lable"><div id="partyNameLabel">
									<sup class="star">* </sup>
									<bean:message key="label.lcCodReport.partyName" />
								</div></td>
							<td width="16%">
<%-- 							<html:select property="to.partyName" --%>
<%-- 									styleId="partyNameList" styleClass="selectBox width130"> --%>
<%-- 									<html:option value=""><bean:message key="label.common.select"/></html:option> --%>
<%-- 									<c:forEach var="type" items="${customerList}" varStatus="loop"> --%>
<%-- 										<html:option value="${type.customerId } "> --%>
<%-- 											<c:out value="${type.customerName }-${ type.customerName} " /> --%>
<%-- 										</html:option> --%>
<%-- 									</c:forEach> --%>
<%-- 								</html:select> --%>
								 <select multiple="multiple" name="partyNameList" id="partyNameList"
								class="selectBox width450">
									<c:forEach var="type" items="${customerList}" varStatus="loop">
										<html:option value="${type.customerId } ">
											<c:out value="${type.customerName }-${ type.customerName} " />
										</html:option>
				...
				<div class="button_containerform">
					<html:button property="print" styleClass="btnintform"
						styleId="printBtn" onclick="printLcCodReportPartyWise();">
				...
				<input type="hidden" name="reportName" id="reportName"
					value="lcCodPartyWise" />
			</div>
		</div>

		<!-- <input type="hidden" name = "reportName" id="reportName"  value="lcCodPartyWise"/>	 -->
