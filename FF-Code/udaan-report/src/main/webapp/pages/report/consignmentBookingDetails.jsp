...
<body onload="triggeringBranchCustomers();">
			...
			<div id="maincontent">
				<div class="mainbody">
					<div class="formbox">
						<h1>
							<bean:message
								key="label.consgBookingReport.consgBookingDetailsReportHeader" />
						</h1>
...
<tr>
	...
	<td><c:choose>
			<c:when test="${wrapperReportTO.getRegionTO().size() == 1}">
				...
				<select name="to.regionTo" id="regionList"
					class="selectBox width300" disabled="disabled">
					<option
			...
			<c:otherwise>
				<select name="to.regionTo" id="regionList"
					class="selectBox width300" 
					onchange="getStationsList('regionList', 'stationList,branchList,destRegionList');clearMultiSelect('customerList','product');">
					<option value="Select">Select</option>
	...
	<td><c:choose>
			...
			<c:when test="${wrapperReportTO.getCityTO().size() == 0}">
				<select name="to.station" id="stationList"
					class="selectBox width300"  onchange="getBranchList('stationList');clearMultiSelect('customerList','product');getCustomerByOffice();">
					<option value="Select">Select</option>
				</select>
	...
	<td><c:choose>
			<c:when test="${wrapperReportTO.getOfficeTO().size() == 1 }">
				<select name="to.station" id="branchList" class="selectBox width130" disabled="disabled"  onchange="clearMultiSelect('customerList','product');getCustomerByOffice();">
			...
			<c:when test="${wrapperReportTO.getOfficeTO().size() == 0 }">
				<select name="to.station" id="branchList" class="selectBox width130"  onchange="clearMultiSelect('customerList','product');getCustomerByOffice();">
					<!-- onchange="getConsignmentBookingClientList('branchList')"> -->
					<option value="Select">Select</option>
				</select>
			</c:when>
		...
		<td class="lable" width="15%"><sup class="star">* </sup> <bean:message
										key="label.consignmentCustomerType.type" /></td>
								<td width="18%"><select id="customerType" class="selectBox width200" onchange="enable();clear();getCustomerByOffice();clearMultiSelect('customerList','product');"
									 >
									<option value="Select">Select</option>
									<!-- <option value="All">All</option> -->
								
									<c:forEach var="type" items="${productTo}">
						<option  value="${type.financialProductId}">
							${type.financialProductName}</option>
					</c:forEach>	
								</select></td>
		<td class="lable" width="15%"><sup class="star">* </sup> Priority</td>
								<td width="18%"><select id="Priority" class="selectBox width200"
									 >
									<option value="Select">Select</option>
									<option value="All">All</option>
									<c:forEach var="priorityType" items="${PriorityTypeTos}">
						<option  value="${priorityType.stdTypeCode}">
							${priorityType.description}</option>
					</c:forEach>	
		</select></td>
</tr>
							<tr>
								...
								<td><select name="to.destRegion" id="destRegionList"
									class="selectBox width300"
									onchange="getMultiStationList(this)" >
										<option>
								...
								<td><select name="to.destStation" id="destStationList"
									class="selectBox width300" onchange="getBranches('originStation',this)" >
										<option>
											<bean:message key="label.common.select" />
										</option>
								</select></td>
							...
							<tr>
								...
								<td>
									...
									class="selectBox width130" multiple="multiple" onmouseup="checkMultiSelectDropdown('client')"> -->
									
									<select name="customerList" id="customerList" class="selectBox width300" size="10"
									multiple="multiple" onchange="getProductByCustomersBD()">
										<option value="Select">
								...
								<td>
									...
									class="selectBox width130" multiple="multiple" onmouseup="checkMultiSelectDropdown('product')"> -->
									
									<%-- <select name="product" id="product" class="selectBox width130"
									multiple="multiple">
										<option>
											<bean:message key="label.all" />
										</option>
									...
									<select name="product" id="product" class="selectBox width300" size="10"
									multiple="multiple">
										<option value="Select">
											<bean:message key="sales.label.select" />
										</option>
			...
			<div class="button_containerform">
				<html:button property="Submit" styleClass="btnintform"
					onclick="showConsignmentBookingDetail();" styleId="Submit">
					<bean:message key="button.submit" />
				</html:button>
				<html:button property="cancel" styleClass="btnintform"
					styleId="cancelBtn" onclick="clearScreen();clearMultiSelect('destStationList','client');clearDestinationFields();clearConsgBookingDetailStation();clearFields('branchList');">
