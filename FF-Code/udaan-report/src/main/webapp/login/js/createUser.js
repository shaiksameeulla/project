...
function searchCustomer(){
	...
	custFormClear();
		...
		var url = './createCustomerLogin.do?submitName=searchCust&cName='+cName;
	...
	if(eName=="")
	{
		...
		if(isChecked) {
			...
			if(selectedName==""){
				...
				var selectedStatus = jQuery("#status"+(i+1)).val();
					empstatus=empstatus+selectedName+"`"+status+","
		...
		if(checkLoginID=="true"||flag=="false"){
			alert('Atleast 1 Login ID should be selected and should not be blank');
				...
				url = './createEmployeeLogin.do?submitName=activateDeactivateEmpLogin&empstatus='+empstatus;
