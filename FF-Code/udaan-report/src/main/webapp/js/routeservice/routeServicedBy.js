	...
	if (!isNull(vendor)) {
		for ( var i = 1; i < rowCount; i++) {
			if (!isNull(getDomElementById("vendorNumber" + i))) {
				if ((rowId != i)
						&& (vendor == getDomElementById("vendorNumber" + i).value)) {
		...
		if (check) {
			...
			ajaxCall(url, "routeServicedByForm", populateTransporterDetails);
		}
...
function saveRouteServicedBy() {
	...
	if (checkGridValues()) {
		...
		if (check) {
			...
			ajaxCall(url, "routeServicedByForm", saveCallback);
		} else {
		...
		if(error!=null){
			...
			if (!isNull(respData)) {				
				...
				if (!isNull(tripList)) {
					...
					alert("Roure does not exist between selected Origin station and Destination Station");
					enableHeaderFields();
					return false;
				}
