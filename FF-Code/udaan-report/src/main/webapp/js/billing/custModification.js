...
function saveOrUpdateCustModification(){
	...
	for(var i=0;i<shipToCodesList.length;i++){	
		if(data[i].trim() == customername.trim()){
			var listOfCustCodes=shipToCodesList[i];
			for(var k=0;k<listOfCustCodes.length;k++){
				if(listOfCustCodes[k].trim() == shipToCode.trim()){
					newcustomerId=custIDArr[i];
					newCustomerCode=custCodeArr[i];
					break;
				}
			}
			if(!isNull(newCustomerCode))
				break;
		}
	}
	if(newcustomerId ==""){
...
function loadNewCustomerList(res){
	...
	$.each( res, function( i, val ) {
		...
		shipToCodesList.push(val.shipedToCodeList);
		var shipToCodelist=val.shipedToCodeList;
		for(var k=0;k<shipToCodelist.length;k++){
			custCodeName.push(val.businessName+" - "+shipToCodelist[k]);
		}	
	});
