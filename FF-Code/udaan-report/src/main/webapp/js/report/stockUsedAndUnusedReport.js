 ...
 * clearScreen
 */
function clearSUScreen(){
	var url="./stockUsedNUnused.do?submitName=getStockUsedAndUnusedReport";
...
function getOrgStationsList(){
	...
	if(region){
	var url = './stockUsedNUnused.do?submitName=getStations&region='
		+ region;
...
function getBranchList(id){
	getDomElementById("branchList").options.length = 1;
		...
		var cityID=$("#"+id).val();	
		if(cityID){
		var url = './stockUsedNUnused.do?submitName=getBranchList&cityID=' + cityID;
...
function getProductList(id){
	...
	var branchId=$("#"+id).val();		
	var url = './stockUsedNUnused.do?submitName=getProductList&branchId=' + branchId;
...
function printReport(){	
	...
	//if(isNull(region)|| isNull(station) || isNull(branch) || isNull(product) ){
	...
	*/
		...
		if(checkStockIssueDateDate()){	
			...
			var url = '/udaan-report/pages/reportviewer/stockUsedAndUnusedViewer.jsp?'			
			 + "&StartDate="+startDate 
			 + "&EndDate="+endDate
			 + "&Region="+region
			 + "&Station="+station
...
function checkStockIssueDateDate(){
	var startDate=$("#startDate" ).val();
	var endDate=$("#endDate" ).val();
...
function getEmployees(id){
	...
	var url = './stockUsedNUnused.do?submitName=getEmployeeList&branchId=' + branchId;
...
function getClientListForStock(id) {

	var officeId = readAllBranchIds();
	var cityId = "";
	var cityIds = document.getElementById("stationList");
	if ($("#stationList").val().toUpperCase() == "ALL") {
		for ( var i = 2; i < cityIds.length; i++) {
			cityId = cityId + cityIds.options[i].value;
			if (i != (cityIds.length - 1)) {
				cityId = cityId + ", ";
			}
		}
	} else {
		cityId = $("#stationList").val();
	}
	var url = './stockUsedNUnused.do?submitName=getCustomerList1&officeId='
			+ officeId + "&cityId=" + cityId;
	ajaxCallWithoutForm(url, populateCustomersForReportList);
}


function readAllBranchIds(){
	var branch = "";
	var branches = document.getElementById("branchList");	
	if($("#branchList").val().toUpperCase() == "ALL"){
		for (var i = 2; i<branches.length; i++){
...
function populateCustomersForReportList(data){
