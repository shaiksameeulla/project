 ...
 * clearScreen
 */
function clearMISScreen(){
	var url="./customerwiseMISReport.do?submitName=getCustomerWiseMISReport";
...
function getCustomerList(id){
	var cityID=$("#"+id).val();	
	var url = './stockUsedNUnused.do?submitName=getCustomerList&cityID=' + cityID;	
...
function printCustomerMISReport(){	
	...
	var frommonth=$("#month1" ).val();
	var tomonth=$("#month2" ).val();			
	...
	if(isNull(region) || isNull(frommonth) || isNull(tomonth) || 'select' ==  region.toLocaleLowerCase() ){
		
	alert('Select Mandatory Field ');
	if(isNull(region) || 'select' ==  region.toLocaleLowerCase()){
		$("#regionList").focus();
	}
	else if(isNull(frommonth)){
			$("#month1").focus();
		}	
		else if(isNull(tomonth)){
			$("#month2").focus();
		}	
		...
		if(checkMonth('month1', 'month2')){	
			...
			var url = '/udaan-report/pages/reportviewer/customerWiseMISViewer.jsp?'	
			 + "&Region="+region
			 + "&Station="+station
			 + "&Branch="+branch			
			 + "&month1="+frommonth
