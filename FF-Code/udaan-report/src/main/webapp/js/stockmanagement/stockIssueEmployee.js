...
function checkMandatoryForIssueEmpSave(type)
{
	...
	if(isNull(fieldRep.value)){
		...
		setTimeout(function(){fieldRep.focus();},10);
		return false;
	}
	if(isNull(qty.value)){
...
function checkStockForIssueEmp(){
	...
	if(isNull(item)){
		...
		setTimeout(function(){itemDom.focus();},10);
		return false;
	}
...
function validateSeries(domElement,type){
	...
	if(isNull(repId)){
		...
		repIdDom.value="";
		setTimeout(function(){repIdDom.focus();},10);
		return false;
	}
	if(isNull(issQnty)){
		clearSeriesWithoutId();
...
function checkSeriesFormat(series){
	...
	//if(!series.startsWith(branchProduct)){
		...
		clearSeriesWithoutId();
		setTimeout(function(){getDomElementById("startSerialNumber").focus();},10);
		return false;
	}
