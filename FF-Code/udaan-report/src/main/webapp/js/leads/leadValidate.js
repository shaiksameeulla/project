...
function leadValidationStartUp(){
	...
	if(userRoleName=="SalesExecutive"){
		dropdownDisable();//in common.js
		allTextBoxDisable();
		//to disable the button also
	} else if(userRoleName=="ControlTeam"){
...
function callBackApproveLead(data){
	...
	/*if (data != null && data!="Failure") {
		...
		alert('Lead not approved successfully.');
	}*/
	}
...
function putOnHold(action){
	...
	function callBackPutOnHold(data){
		...
		/*
		  if (data != null && data!="Failure") { 
			  alert('Lead is put on hold  successfully.'); 
			  } else { 
...
function callBackRejectLead(data){
	...
	/*
	 * if (data != null && data!="Failure") { alert('Lead rejected
	 * successfully.'); //disableForSave(); } else { alert('Lead could not be
...
function emailPopUp(){
	...
	var url = './sendEmail.do?submitName=preparePage&leadNumber='+ leadNumber;
		window
		.open(url,'newWindow','toolbar=0,scrollbars=0,location=0,statusbar=0,menubar=0,resizable=0,width=1000,height=500,left = 200,top = 184');
		//document.createLeadForm.action=url;
		//document.createLeadForm.submit();
}
function SMSPopUp(){
